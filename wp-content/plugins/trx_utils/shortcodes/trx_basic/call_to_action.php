...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_call_to_action_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_call_to_action_theme_setup' );
	function melodyschool_sc_call_to_action_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_call_to_action_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_call_to_action')) {	
	function melodyschool_sc_call_to_action($atts, $content=null){	
		...
		if (!empty($image)) {
			$thumb_sizes = melodyschool_get_thumb_sizes(array('layout' => 'excerpt'));
			$image = !empty($video) 
				? melodyschool_get_resized_image_url($image, $thumb_sizes['w'], $thumb_sizes['h']) 
				: melodyschool_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
	
		if (!empty($video)) {
			$video = '<video' . ($id ? ' id="' . esc_attr($id.'_video') . '"' : '') 
				. ' class="sc_video"'
				. ' src="' . esc_url(melodyschool_get_video_player_url($video)) . '"'
				...
				. '>'
				. '</video>';
			if (melodyschool_get_custom_option('substitute_video')=='no') {
				$video = melodyschool_get_video_frame($video, $image, '', '');
			} else {
				if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
					$video = melodyschool_substitute_video($video, $width, $height, false);
				}
			}
		...
		$buttons = (!empty($link) || !empty($link2) 
						? '<div class="sc_call_to_action_buttons sc_item_buttons'.($need_columns && $style==2 ? ' column-1_'.esc_attr($need_columns) : '').'">'
							. (!empty($link) 
								? '<div class="sc_call_to_action_button sc_item_button">'.do_shortcode('[trx_button link="'.esc_url($link).'" class="style_color_dark_hover"]'.esc_html($link_caption).'[/trx_button]').'</div>'
								: '')
							. (!empty($link2) 
								? '<div class="sc_call_to_action_button sc_item_button">'.do_shortcode('[trx_button link="'.esc_url($link2).'" class="style_color_dark_hover"]'.esc_html($link2_caption).'[/trx_button]').'</div>'
								: '')
							. '</div>'
						: '');
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (melodyschool_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
					. ' sc_call_to_action_style_' . esc_attr($style) 
					. ' sc_call_to_action_align_'.esc_attr($align)
					. (!empty($class) ? ' '.esc_attr($class) : '')
                    . (!empty($color_style) ? ' '.esc_attr($color_style) : '')
					. '"'
			...
			. '>'
				...
				. '<div class="sc_call_to_action_info'.($need_columns ? ' column-'.esc_attr($need_columns-1).'_'.esc_attr($need_columns) : '').'">'
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(melodyschool_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h1 class="sc_call_to_action_title sc_item_title">' . trim(melodyschool_strmacros($title)) . '</h1>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(melodyschool_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
				. ($align=='right' ? $featured : '')
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	add_shortcode('trx_call_to_action', 'melodyschool_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_call_to_action_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_call_to_action_reg_shortcodes');
	function melodyschool_sc_call_to_action_reg_shortcodes() {
	
		melodyschool_sc_map("trx_call_to_action", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_call_to_action_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_call_to_action_reg_shortcodes_vc');
	function melodyschool_sc_call_to_action_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
                ...
                array(
                    ...
                    "heading" => esc_html__("Color style", 'trx_utils'),
                    "description" => wp_kses_data( __("Select color style", 'trx_utils') ),
                    "class" => "",
                    "value" => array(
                        esc_html__('Original', 'trx_utils') => 'color_style_original',
                        esc_html__('Light', 'trx_utils') => 'color_style_light'
                    ),
                    "type" => "dropdown"
                ),
				array(
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends MELODYSCHOOL_VC_ShortCodeCollection {}
	}
}
?>