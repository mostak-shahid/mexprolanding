...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_twitter_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_twitter_theme_setup' );
	function melodyschool_sc_twitter_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_twitter_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_twitter_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_twitter')) {	
	function melodyschool_sc_twitter($atts, $content=null){	
		...
		$twitter_username = $user ? $user : melodyschool_get_theme_option('twitter_username');
		$twitter_consumer_key = $consumer_key ? $consumer_key : melodyschool_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = $consumer_secret ? $consumer_secret : melodyschool_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = $token_key ? $token_key : melodyschool_get_theme_option('twitter_token_key');
		$twitter_token_secret = $token_secret ? $token_secret : melodyschool_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, $count ? $count : intval(melodyschool_get_theme_option('twitter_count')));
		...
		if (!empty($twitter_consumer_key) && !empty($twitter_consumer_secret) && !empty($twitter_token_key) && !empty($twitter_token_secret)) {
			$data = melodyschool_get_twitter_data(array(
				'mode'            => 'user_timeline',
			...
			if ($data && isset($data[0]['text'])) {
				melodyschool_enqueue_slider('swiper');
				$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || melodyschool_strlen($bg_texture)>2 || ($scheme && !melodyschool_param_is_off($scheme) && !melodyschool_param_is_inherit($scheme))
						? '<div class="sc_twitter_wrap sc_section'
								...
								. ($align && $align!='none' && !melodyschool_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
							.' style="'
						...
						. '<div class="sc_twitter'
								...
								. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && $align && $align!='none' && !melodyschool_param_is_inherit($align) ? ' align' . esc_attr($align) : '')
								. '"'
				...
				if (is_array($data) && count($data) > 0) {
					foreach ($data as $tweet) {
						if (melodyschool_substr($tweet['text'], 0, 1)=='@') continue;
							$output .= '<div class="swiper-slide" data-style="'.esc_attr(($ws).($hs)).'" style="'.esc_attr(($ws).($hs)).'">'
										. '<div class="sc_twitter_item">'
											...
											. '<div class="sc_twitter_content">'
												...
												. force_balance_tags(melodyschool_prepare_twitter_text($tweet))
											. '</div>'
										. '</div>'
									. '</div>';
				...
				$output .= '</div>'
						...
						. '</div>'
					. '</div>'
					. ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || melodyschool_strlen($bg_texture)>2
						?  '</div></div>'
						: '');
			}
		}
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_twitter', $atts, $content);
	}
	add_shortcode('trx_twitter', 'melodyschool_sc_twitter');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_twitter_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_twitter_reg_shortcodes');
	function melodyschool_sc_twitter_reg_shortcodes() {
	
		melodyschool_sc_map("trx_twitter", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_twitter_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_twitter_reg_shortcodes_vc');
	function melodyschool_sc_twitter_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Twitter extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>