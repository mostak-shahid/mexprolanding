<?php
/**
 * MelodySchool Framework: Clients support
 *
...
// Theme init
if (!function_exists('melodyschool_clients_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_clients_theme_setup', 1 );
	function melodyschool_clients_theme_setup() {

		// Detect current page type, taxonomy and title (for custom post_types use priority < 10 to fire it handles early, than for standard post types)
		add_filter('melodyschool_filter_get_blog_type',			'melodyschool_clients_get_blog_type', 9, 2);
		add_filter('melodyschool_filter_get_blog_title',		'melodyschool_clients_get_blog_title', 9, 2);
		add_filter('melodyschool_filter_get_current_taxonomy',	'melodyschool_clients_get_current_taxonomy', 9, 2);
		add_filter('melodyschool_filter_is_taxonomy',			'melodyschool_clients_is_taxonomy', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_title',	'melodyschool_clients_get_stream_page_title', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_link',	'melodyschool_clients_get_stream_page_link', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_id',	'melodyschool_clients_get_stream_page_id', 9, 2);
		add_filter('melodyschool_filter_query_add_filters',		'melodyschool_clients_query_add_filters', 9, 2);
		add_filter('melodyschool_filter_detect_inheritance_key','melodyschool_clients_detect_inheritance_key', 9, 1);

		// Extra column for clients lists
		if (melodyschool_get_theme_option('show_overriden_posts')=='yes') {
			add_filter('manage_edit-clients_columns',			'melodyschool_post_add_options_column', 9);
			add_filter('manage_clients_posts_custom_column',	'melodyschool_post_fill_options_column', 9, 2);
		}

		// Registar shortcodes [trx_clients] and [trx_clients_item] in the shortcodes list
		add_action('melodyschool_action_shortcodes_list',		'melodyschool_clients_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_clients_reg_shortcodes_vc');
		
		// Add supported data types
		melodyschool_theme_support_pt('clients');
		melodyschool_theme_support_tx('clients_group');
	}
}

if ( !function_exists( 'melodyschool_clients_settings_theme_setup2' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_clients_settings_theme_setup2', 3 );
	function melodyschool_clients_settings_theme_setup2() {
		// Add post type 'clients' and taxonomy 'clients_group' into theme inheritance list
		melodyschool_add_theme_inheritance( array('clients' => array(
...
if (!function_exists('melodyschool_clients_after_theme_setup')) {
	add_action( 'melodyschool_action_after_init_theme', 'melodyschool_clients_after_theme_setup' );
	function melodyschool_clients_after_theme_setup() {
		// Update fields in the override options
		if (melodyschool_storage_get_array('post_override_options', 'page')=='clients') {
			// Meta box fields
			melodyschool_storage_set_array('post_override_options', 'title', esc_html__('Client Options', 'trx_utils'));
...
if ( !function_exists( 'melodyschool_is_clients_page' ) ) {
	function melodyschool_is_clients_page() {
		$is = in_array(melodyschool_storage_get('page_template'), array('blog-clients', 'single-client'));
		if (!$is) {
			if (!melodyschool_storage_empty('pre_query'))
				$is = melodyschool_storage_call_obj_method('pre_query', 'get', 'post_type')=='clients'
						|| melodyschool_storage_call_obj_method('pre_query', 'is_tax', 'clients_group') 
						|| (melodyschool_storage_call_obj_method('pre_query', 'is_page') 
							&& ($id=melodyschool_get_template_page_id('blog-clients')) > 0 
				...
				$is = get_query_var('post_type')=='clients' 
						...
						|| (is_page() && ($id=melodyschool_get_template_page_id('blog-clients')) > 0 && $id==get_the_ID());
		}
		return $is;
	}
}
...
if ( !function_exists( 'melodyschool_clients_detect_inheritance_key' ) ) {
	//Handler of add_filter('melodyschool_filter_detect_inheritance_key',	'melodyschool_clients_detect_inheritance_key', 9, 1);
	function melodyschool_clients_detect_inheritance_key($key) {
		if (!empty($key)) return $key;
		return melodyschool_is_clients_page() ? 'clients' : '';
	}
}

// Filter to detect current page slug
if ( !function_exists( 'melodyschool_clients_get_blog_type' ) ) {
	//Handler of add_filter('melodyschool_filter_get_blog_type',	'melodyschool_clients_get_blog_type', 9, 2);
	function melodyschool_clients_get_blog_type($page, $query=null) {
...
if ( !function_exists( 'melodyschool_clients_get_blog_title' ) ) {
	//Handler of add_filter('melodyschool_filter_get_blog_title',	'melodyschool_clients_get_blog_title', 9, 2);
	function melodyschool_clients_get_blog_title($title, $page) {
		if (!empty($title)) return $title;
		if ( melodyschool_strpos($page, 'clients')!==false ) {
...
if ( !function_exists( 'melodyschool_clients_get_stream_page_title' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_title',	'melodyschool_clients_get_stream_page_title', 9, 2);
	function melodyschool_clients_get_stream_page_title($title, $page) {
		if (!empty($title)) return $title;
		if (melodyschool_strpos($page, 'clients')!==false) {
			if (($page_id = melodyschool_clients_get_stream_page_id(0, $page=='clients' ? 'blog-clients' : $page)) > 0)
...
if ( !function_exists( 'melodyschool_clients_get_stream_page_id' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_id',	'melodyschool_clients_get_stream_page_id', 9, 2);
	function melodyschool_clients_get_stream_page_id($id, $page) {
		if (!empty($id)) return $id;
		if (melodyschool_strpos($page, 'clients')!==false) $id = melodyschool_get_template_page_id('blog-clients');
		return $id;
	}
}

// Filter to detect stream page URL
if ( !function_exists( 'melodyschool_clients_get_stream_page_link' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_link',	'melodyschool_clients_get_stream_page_link', 9, 2);
	function melodyschool_clients_get_stream_page_link($url, $page) {
		if (!empty($url)) return $url;
		if (melodyschool_strpos($page, 'clients')!==false) {
			$id = melodyschool_get_template_page_id('blog-clients');
...
if ( !function_exists( 'melodyschool_clients_get_current_taxonomy' ) ) {
	//Handler of add_filter('melodyschool_filter_get_current_taxonomy',	'melodyschool_clients_get_current_taxonomy', 9, 2);
	function melodyschool_clients_get_current_taxonomy($tax, $page) {
...
if ( !function_exists( 'melodyschool_clients_is_taxonomy' ) ) {
	//Handler of add_filter('melodyschool_filter_is_taxonomy',	'melodyschool_clients_is_taxonomy', 9, 2);
	function melodyschool_clients_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			return $tax;
		else 
...
if ( !function_exists( 'melodyschool_clients_query_add_filters' ) ) {
	//Handler of add_filter('melodyschool_filter_query_add_filters',	'melodyschool_clients_query_add_filters', 9, 2);
	function melodyschool_clients_query_add_filters($args, $filter) {
		if ($filter == 'clients') {
...
[/trx_clients]
*/
if ( !function_exists( 'melodyschool_sc_clients' ) ) {
	function melodyschool_sc_clients($atts, $content=null){	
		...
		melodyschool_storage_set('sc_clients_data', array(
			'id'=>$id,
            'style'=>$style,
            'counter'=>0,
            'columns'=>$columns,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '') 
					...
					. '>'
					. (!empty($subtitle) ? '<h6 class="sc_clients_subtitle sc_item_subtitle">' . trim(melodyschool_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_clients_title sc_item_title">' . trim(melodyschool_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_clients_descr sc_item_descr">' . trim(melodyschool_strmacros($description)) . '</div>' : '')
		...
		if (melodyschool_param_is_on($custom) && $content) {
			...
			$args = melodyschool_query_add_posts_and_cats($args, $ids, 'clients', $cat, 'clients_group');
			...
			while ( $query->have_posts() ) { 
				...
				$post_meta = get_post_meta($post_data['post_id'], melodyschool_storage_get('options_prefix') . '_post_options', true);
				...
				$args['client_link'] = melodyschool_param_is_on('client_show_link')
		...
		$output .= (!empty($link) ? '<div class="sc_clients_button sc_item_button">'.melodyschool_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
				. '</div><!-- /.sc_clients -->'
			. '</div><!-- /.sc_clients_wrap -->';
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_clients', $atts, $content);
	}
	add_shortcode('trx_clients', 'melodyschool_sc_clients');
}


if ( !function_exists( 'melodyschool_sc_clients_item' ) ) {
	function melodyschool_sc_clients_item($atts, $content=null) {
		if (melodyschool_in_shortcode_blogger()) return '';
		extract(melodyschool_html_decode(shortcode_atts( array(
			// Individual params
			"name" => "",
		...
		melodyschool_storage_inc_array('sc_clients_data', 'counter');
	
		$id = $id ? $id : (melodyschool_storage_get_array('sc_clients_data', 'id') ? melodyschool_storage_get_array('sc_clients_data', 'id') . '_' . melodyschool_storage_get_array('sc_clients_data', 'counter') : '');
	
		$descr = trim(chop(do_shortcode($content)));
	
		$thumb_sizes = melodyschool_get_thumb_sizes(array('layout' => melodyschool_storage_get_array('sc_clients_data', 'style')));
		...
		$image = melodyschool_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		...
		$args = array(
			'layout' => melodyschool_storage_get_array('sc_clients_data', 'style'),
			'number' => melodyschool_storage_get_array('sc_clients_data', 'counter'),
			'columns_count' => melodyschool_storage_get_array('sc_clients_data', 'columns'),
			'slider' => melodyschool_storage_get_array('sc_clients_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			...
			'tag_css' => $css,
			'tag_css_wh' => melodyschool_storage_get_array('sc_clients_data', 'css_wh'),
			'client_position' => $position,
			'client_link' => $link,
			'client_image' => $image
		);
		$output = melodyschool_show_post_layout($args, $post_data);
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_clients_item', $atts, $content);
	}
	add_shortcode('trx_clients_item', 'melodyschool_sc_clients_item');
}
// ---------------------------------- [/trx_clients] ---------------------------------------
...
if (!function_exists('melodyschool_clients_reg_shortcodes')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list',	'melodyschool_clients_reg_shortcodes');
	function melodyschool_clients_reg_shortcodes() {
		if (melodyschool_storage_isset('shortcodes')) {
			...
			$clients_groups = melodyschool_get_list_terms(false, 'clients_group');
			$clients_styles = melodyschool_get_list_templates('clients');
			$controls 		= melodyschool_get_list_slider_controls();

			melodyschool_sc_map_after('trx_chat', array(
				...
				"trx_clients" => array(
					...
					"params" => array(
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $clients_groups)
						),
						"count" => array(
...
if (!function_exists('melodyschool_clients_reg_shortcodes_vc')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list_vc',	'melodyschool_clients_reg_shortcodes_vc');
	function melodyschool_clients_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"class" => "",
						"value" => array_flip(melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $clients_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
		...
		class WPBakeryShortCode_Trx_Clients extends MELODYSCHOOL_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Clients_Item extends MELODYSCHOOL_VC_ShortCodeCollection {}
