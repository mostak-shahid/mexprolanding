...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_line_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_line_theme_setup' );
	function melodyschool_sc_line_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_line_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_line_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_line')) {	
	function melodyschool_sc_line($atts, $content=null){	
		...
		if ($style=='image' && !empty($image)) {
			...
			$attr = melodyschool_getimagesize($image);
		...
		$css .= melodyschool_get_css_dimensions_from_values($width, $block_height)
			. ($style=='image' && !empty($image)
				? ( 'background-image: url(' . esc_url($image) . ');'
					. (melodyschool_param_is_on($repeat) ? 'background-repeat: repeat-x;' : '')
					)
				: ( ($height !='' ? 'border-'.esc_attr($border_pos).'-width:' . esc_attr(melodyschool_prepare_css_value($height)) . ';' : '')
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id) . '"' : '') 
				...
				. '</div>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_line', $atts, $content);
	}
	add_shortcode('trx_line', 'melodyschool_sc_line');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_line_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_line_reg_shortcodes');
	function melodyschool_sc_line_reg_shortcodes() {
	
		melodyschool_sc_map("trx_line", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "solid",
					"dir" => "horizontal",
					"options" => melodyschool_get_list_line_styles(),
					"type" => "checklist"
				),
				"image" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_line_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_line_reg_shortcodes_vc');
	function melodyschool_sc_line_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "solid",
					"value" => array_flip(melodyschool_get_list_line_styles()),
					"type" => "dropdown"
				),
				array(
					"param_name" => "image",
				...
				array(
					...
					"class" => "",
					"std" => "center center",
					"value" => array_flip(melodyschool_get_list_bg_image_positions()),
					"type" => "dropdown"
				),
		...
		class WPBakeryShortCode_Trx_Line extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>