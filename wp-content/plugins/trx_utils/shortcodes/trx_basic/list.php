...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_list_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_list_theme_setup' );
	function melodyschool_sc_list_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_list_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_list_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_list')) {	
	function melodyschool_sc_list($atts, $content=null){	
		...
		melodyschool_storage_set('sc_list_data', array(
			'counter' => 0,
            'icon' => empty($icon) || melodyschool_param_is_inherit($icon) ? "icon-right" : $icon,
		...
		$output = '<' . ($style=='ol' ? 'ol' : 'ul')
				...
				. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_list', $atts, $content);
	}
	add_shortcode('trx_list', 'melodyschool_sc_list');
}


if (!function_exists('melodyschool_sc_list_item')) {	
	function melodyschool_sc_list_item($atts, $content=null) {
		if (melodyschool_in_shortcode_blogger()) return '';
		extract(melodyschool_html_decode(shortcode_atts( array(
			// Individual params
			"color" => "",
		...
		melodyschool_storage_inc_array('sc_list_data', 'counter');
		$css .= $color !== '' ? 'color:' . esc_attr($color) .';' : '';
		if (trim($icon) == '' || melodyschool_param_is_inherit($icon)) $icon = melodyschool_storage_get_array('sc_list_data', 'icon');
		if (trim($color) == '' || melodyschool_param_is_inherit($icon_color)) $icon_color = melodyschool_storage_get_array('sc_list_data', 'icon_color');
		$content = do_shortcode($content);
		if (empty($content)) $content = $title;
		$output = '<li' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			. ' class="sc_list_item' 
			. (!empty($class) ? ' '.esc_attr($class) : '')
			. (melodyschool_storage_get_array('sc_list_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
			. (melodyschool_storage_get_array('sc_list_data', 'counter') == 1 ? ' first' : '')  
			. '"' 
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. ($title ? ' title="'.esc_attr($title).'"' : '') 
			. '>' 
			. (!empty($link) ? '<a href="'.esc_url($link).'"' . (!empty($target) ? ' target="'.esc_attr($target).'"' : '') . '>' : '')
			. (melodyschool_storage_get_array('sc_list_data', 'style')=='iconed' && $icon!='' ? '<span class="sc_list_icon '.esc_attr($icon).'"'.($icon_color !== '' ? ' style="color:'.esc_attr($icon_color).';"' : '').'></span>' : '')
			. trim($content)
			. (!empty($link) ? '</a>': '')
			. '</li>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_list_item', $atts, $content);
	}
	add_shortcode('trx_list_item', 'melodyschool_sc_list_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_list_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_list_reg_shortcodes');
	function melodyschool_sc_list_reg_shortcodes() {
	
		melodyschool_sc_map("trx_list", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "ul",
					"type" => "checklist",
					"options" => melodyschool_get_sc_param('list_styles')
				), 
				"color" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_list_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_list_reg_shortcodes_vc');
	function melodyschool_sc_list_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"admin_label" => true,
					"value" => array_flip(melodyschool_get_sc_param('list_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "color",
		...
		class WPBakeryShortCode_Trx_List extends MELODYSCHOOL_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_List_Item extends MELODYSCHOOL_VC_ShortCodeContainer {}
	}
}
?>