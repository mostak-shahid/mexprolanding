<?php
/**
 * MelodySchool Framework: Team support
 *
...
// Theme init
if (!function_exists('melodyschool_team_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_team_theme_setup', 1 );
	function melodyschool_team_theme_setup() {

		// Add item in the admin menu
		add_filter('trx_utils_filter_override_options',						'melodyschool_team_add_override_options');

		// Save data from override options
		add_action('save_post',								'melodyschool_team_save_data');
		
		// Detect current page type, taxonomy and title (for custom post_types use priority < 10 to fire it handles early, than for standard post types)
		add_filter('melodyschool_filter_get_blog_type',			'melodyschool_team_get_blog_type', 9, 2);
		add_filter('melodyschool_filter_get_blog_title',		'melodyschool_team_get_blog_title', 9, 2);
		add_filter('melodyschool_filter_get_current_taxonomy',	'melodyschool_team_get_current_taxonomy', 9, 2);
		add_filter('melodyschool_filter_is_taxonomy',			'melodyschool_team_is_taxonomy', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_title',	'melodyschool_team_get_stream_page_title', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_link',	'melodyschool_team_get_stream_page_link', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_id',	'melodyschool_team_get_stream_page_id', 9, 2);
		add_filter('melodyschool_filter_query_add_filters',		'melodyschool_team_query_add_filters', 9, 2);
		add_filter('melodyschool_filter_detect_inheritance_key','melodyschool_team_detect_inheritance_key', 9, 1);

		// Extra column for team members lists
		if (melodyschool_get_theme_option('show_overriden_posts')=='yes') {
			add_filter('manage_edit-team_columns',			'melodyschool_post_add_options_column', 9);
			add_filter('manage_team_posts_custom_column',	'melodyschool_post_fill_options_column', 9, 2);
		}

		// Register shortcodes [trx_team] and [trx_team_item]
		add_action('melodyschool_action_shortcodes_list',		'melodyschool_team_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_team_reg_shortcodes_vc');

		// Meta box fields
		melodyschool_storage_set('team_override_options', array(
		...
		melodyschool_theme_support_pt('team');
		melodyschool_theme_support_tx('team_group');
	}
}

if ( !function_exists( 'melodyschool_team_settings_theme_setup2' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_team_settings_theme_setup2', 3 );
	function melodyschool_team_settings_theme_setup2() {
		// Add post type 'team' and taxonomy 'team_group' into theme inheritance list
		melodyschool_add_theme_inheritance( array('team' => array(
...
if (!function_exists('melodyschool_team_add_override_options')) {
    //add_action('trx_utils_filter_override_options', 'melodyschool_team_add_override_options');
    function melodyschool_team_add_override_options($boxes = array()) {
        $boxes[] = array_merge(melodyschool_storage_get('team_override_options'), array('callback' => 'melodyschool_team_show_override_options'));
        return $boxes;
    }
}
...
if (!function_exists('melodyschool_team_show_override_options')) {
	function melodyschool_team_show_override_options() {
		global $post;

		$data = get_post_meta($post->ID, melodyschool_storage_get('options_prefix').'_team_data', true);
		$fields = melodyschool_storage_get_array('team_override_options', 'fields');
		?>
						...
						if ($id == 'team_member_socials') {
							...
							if (is_array($social_list) && count($social_list) > 0) {
								foreach ($social_list as $soc) {
									...
									?>
									<label for="<?php echo esc_attr(($id).'_'.($sn)); ?>"><?php echo esc_attr(melodyschool_strtoproper($sn)); ?></label><br>
...
// Save data from override options
if (!function_exists('melodyschool_team_save_data')) {
	//Handler of add_action('save_post', 'melodyschool_team_save_data');
	function melodyschool_team_save_data($post_id) {
		// verify nonce
		if ( !wp_verify_nonce( melodyschool_get_value_gp('override_options_team_nonce'), admin_url() ) )
			return $post_id;
		...
		update_post_meta($post_id, melodyschool_storage_get('options_prefix').'_team_data', $data);
	}
}
...
if ( !function_exists( 'melodyschool_is_team_page' ) ) {
	function melodyschool_is_team_page() {
		$is = in_array(melodyschool_storage_get('page_template'), array('blog-team', 'single-team'));
		if (!$is) {
			if (!melodyschool_storage_empty('pre_query'))
				$is = melodyschool_storage_call_obj_method('pre_query', 'get', 'post_type')=='team' 
						|| melodyschool_storage_call_obj_method('pre_query', 'is_tax', 'team_group') 
						|| (melodyschool_storage_call_obj_method('pre_query', 'is_page') 
								&& ($id=melodyschool_get_template_page_id('blog-team')) > 0 
								&& $id==melodyschool_storage_get_obj_property('pre_query', 'queried_object_id', 0)
							);
			else
				$is = get_query_var('post_type')=='team' || is_tax('team_group') || (is_page() && ($id=melodyschool_get_template_page_id('blog-team')) > 0 && $id==get_the_ID());
		}
		return $is;
	}
}
...
if ( !function_exists( 'melodyschool_team_detect_inheritance_key' ) ) {
	//Handler of add_filter('melodyschool_filter_detect_inheritance_key',	'melodyschool_team_detect_inheritance_key', 9, 1);
	function melodyschool_team_detect_inheritance_key($key) {
		if (!empty($key)) return $key;
		return melodyschool_is_team_page() ? 'team' : '';
	}
}

// Filter to detect current page slug
if ( !function_exists( 'melodyschool_team_get_blog_type' ) ) {
	//Handler of add_filter('melodyschool_filter_get_blog_type',	'melodyschool_team_get_blog_type', 9, 2);
	function melodyschool_team_get_blog_type($page, $query=null) {
...
if ( !function_exists( 'melodyschool_team_get_blog_title' ) ) {
	//Handler of add_filter('melodyschool_filter_get_blog_title',	'melodyschool_team_get_blog_title', 9, 2);
	function melodyschool_team_get_blog_title($title, $page) {
		if (!empty($title)) return $title;
		if ( melodyschool_strpos($page, 'team')!==false ) {
...
if ( !function_exists( 'melodyschool_team_get_stream_page_title' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_title',	'melodyschool_team_get_stream_page_title', 9, 2);
	function melodyschool_team_get_stream_page_title($title, $page) {
		if (!empty($title)) return $title;
		if (melodyschool_strpos($page, 'team')!==false) {
			if (($page_id = melodyschool_team_get_stream_page_id(0, $page=='team' ? 'blog-team' : $page)) > 0)
...
if ( !function_exists( 'melodyschool_team_get_stream_page_id' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_id',	'melodyschool_team_get_stream_page_id', 9, 2);
	function melodyschool_team_get_stream_page_id($id, $page) {
		if (!empty($id)) return $id;
		if (melodyschool_strpos($page, 'team')!==false) $id = melodyschool_get_template_page_id('blog-team');
		return $id;
	}
}

// Filter to detect stream page URL
if ( !function_exists( 'melodyschool_team_get_stream_page_link' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_link',	'melodyschool_team_get_stream_page_link', 9, 2);
	function melodyschool_team_get_stream_page_link($url, $page) {
		if (!empty($url)) return $url;
		if (melodyschool_strpos($page, 'team')!==false) {
			$id = melodyschool_get_template_page_id('blog-team');
...
if ( !function_exists( 'melodyschool_team_get_current_taxonomy' ) ) {
	//Handler of add_filter('melodyschool_filter_get_current_taxonomy',	'melodyschool_team_get_current_taxonomy', 9, 2);
	function melodyschool_team_get_current_taxonomy($tax, $page) {
...
if ( !function_exists( 'melodyschool_team_is_taxonomy' ) ) {
	//Handler of add_filter('melodyschool_filter_is_taxonomy',	'melodyschool_team_is_taxonomy', 9, 2);
	function melodyschool_team_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			return $tax;
		else 
...
if ( !function_exists( 'melodyschool_team_query_add_filters' ) ) {
	//Handler of add_filter('melodyschool_filter_query_add_filters',	'melodyschool_team_query_add_filters', 9, 2);
	function melodyschool_team_query_add_filters($args, $filter) {
		if ($filter == 'team') {
			$args['post_type'] = 'team';
		}
		return $args;
...
[/trx_team]
*/
if ( !function_exists( 'melodyschool_sc_team' ) ) {
	function melodyschool_sc_team($atts, $content=null){	
		...
		if (!empty($height) && melodyschool_param_is_on($autoheight)) $autoheight = "no";
		if (empty($interval)) $interval = mt_rand(5000, 10000);

		$class .= ($class ? ' ' : '') . melodyschool_get_css_position_as_classes($top, $right, $bottom, $left);

		$ws = melodyschool_get_css_dimensions_from_values($width);
		$hs = melodyschool_get_css_dimensions_from_values('', $height);
		$css .= ($hs) . ($ws);

		$count = max(1, (int) $count);
		$columns = max(1, min(12, (int) $columns));
		if (melodyschool_param_is_off($custom) && $count < $columns) $columns = $count;

		melodyschool_storage_set('sc_team_data', array(
			'id' => $id,
            'style' => $style,
            'columns' => $columns,
            'counter' => 0,
		...
		if (melodyschool_param_is_on($slider)) melodyschool_enqueue_slider('swiper');
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '') 
					...
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						. ' class="sc_team sc_team_style_'.esc_attr($style)
							. ' ' . esc_attr(melodyschool_get_template_property($style, 'container_classes'))
					...
					. '>'
					. (!empty($subtitle) ? '<h6 class="sc_team_subtitle sc_item_subtitle">' . trim(melodyschool_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_team_title sc_item_title">' . trim(melodyschool_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_team_descr sc_item_descr">' . trim(melodyschool_strmacros($description)) . '</div>' : '')
					. (melodyschool_param_is_on($slider) 
						? ('<div class="sc_slider_swiper swiper-slider-container'
										. ' ' . esc_attr(melodyschool_get_slider_controls_classes($controls))
										. (melodyschool_param_is_on($autoheight) ? ' sc_slider_height_auto' : '')
										. ($hs ? ' sc_slider_height_fixed' : '')
										. '"'
									. (!empty($width) && melodyschool_strpos($width, '%')===false ? ' data-old-width="' . esc_attr($width) . '"' : '')
									. (!empty($height) && melodyschool_strpos($height, '%')===false ? ' data-old-height="' . esc_attr($height) . '"' : '')
						...
						: ($columns > 1 // && melodyschool_get_template_property($style, 'need_columns')
		...
		if (melodyschool_param_is_on($custom) && $content) {
			$output .= $content;
		} else {
			global $post;
			...
			$args = melodyschool_query_add_sort_order($args, $orderby, $order);
			$args = melodyschool_query_add_posts_and_cats($args, $ids, 'team', $cat, 'team_group');
			...
			while ( $query->have_posts() ) { 
				...
				$args = array(
					...
					"descr" => melodyschool_get_custom_option('post_excerpt_maxlength'.($columns > 1 ? '_masonry' : '')),
					"orderby" => $orderby,
					'content' => false,
					'terms_list' => false,
				...
				$post_data = melodyschool_get_post_data($args);
				$post_meta = get_post_meta($post_data['post_id'], melodyschool_storage_get('options_prefix').'_team_data', true);
				$thumb_sizes = melodyschool_get_thumb_sizes(array('layout' => $style));
				...
				if (is_array($soc_list) && count($soc_list)>0) {
					...
					if (!empty($soc_str))
						$args['socials'] = melodyschool_do_shortcode('[trx_socials size="tiny" shape="round" socials="'.esc_attr($soc_str).'"][/trx_socials]');
				}
	
				$output .= melodyschool_show_post_layout($args, $post_data);
			}
			wp_reset_postdata();
		}

		if (melodyschool_param_is_on($slider)) {
			$output .= '</div>'
				. '<div class="sc_slider_controls_wrap"><a class="sc_slider_prev" href="#"></a><a class="sc_slider_next" href="#"></a></div>'
				. '<div class="sc_slider_pagination_wrap"></div>'
				. '</div>';
		} else if ($columns > 1) {// && melodyschool_get_template_property($style, 'need_columns')) {
			$output .= '</div>';
		}

		$output .= (!empty($link) ? '<div class="sc_team_button sc_item_button">'.melodyschool_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
					. '</div><!-- /.sc_team -->'
				. '</div><!-- /.sc_team_wrap -->';
	
		// Add template specific scripts and styles
		do_action('melodyschool_action_blog_scripts', $style);
	
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_team', $atts, $content);
	}
	add_shortcode('trx_team', 'melodyschool_sc_team');
}


if ( !function_exists( 'melodyschool_sc_team_item' ) ) {
	function melodyschool_sc_team_item($atts, $content=null) {
		if (melodyschool_in_shortcode_blogger()) return '';
		extract(melodyschool_html_decode(shortcode_atts( array(
			// Individual params
			"user" => "",
		...
		melodyschool_storage_inc_array('sc_team_data', 'counter');
	
		$id = $id ? $id : (melodyschool_storage_get_array('sc_team_data', 'id') ? melodyschool_storage_get_array('sc_team_data', 'id') . '_' . melodyschool_storage_get_array('sc_team_data', 'counter') : '');
	
		$descr = trim(chop(do_shortcode($content)));
	
		$thumb_sizes = melodyschool_get_thumb_sizes(array('layout' => melodyschool_storage_get_array('sc_team_data', 'style')));
	
		if (!empty($socials)) $socials = melodyschool_do_shortcode('[trx_socials size="tiny" shape="round" socials="'.esc_attr($socials).'"][/trx_socials]');
		...
		if (!empty($user) && $user!='none' && ($user_obj = get_user_by('login', $user)) != false) {
			...
			if (empty($socials))	$socials = melodyschool_show_user_socials(array('author_id'=>$user_obj->ID, 'echo'=>false));
		}
		...
		if (!empty($member) && $member!='none' && ($member_obj = (intval($member) > 0 ? get_post($member, OBJECT) : get_page_by_title($member, OBJECT, 'team'))) != null) {
			...
			$post_meta = get_post_meta($member_obj->ID, melodyschool_storage_get('options_prefix').'_team_data', true);
			...
			if (empty($socials)) {
				...
				if (is_array($soc_list) && count($soc_list)>0) {
					...
					if (!empty($soc_str))
						$socials = melodyschool_do_shortcode('[trx_socials size="tiny" shape="round" socials="'.esc_attr($soc_str).'"][/trx_socials]');
				}
			}
		}
		if (empty($photo)) {
			...
			$photo = melodyschool_get_resized_image_tag($photo, $thumb_sizes['w'], $thumb_sizes['h']);
		}
		$post_data = array(
			'post_title' => $name,
			'post_excerpt' => $descr
		...
		$args = array(
			'layout' => melodyschool_storage_get_array('sc_team_data', 'style'),
			'number' => melodyschool_storage_get_array('sc_team_data', 'counter'),
			'columns_count' => melodyschool_storage_get_array('sc_team_data', 'columns'),
			'slider' => melodyschool_storage_get_array('sc_team_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			...
			'tag_css' => $css,
			'tag_css_wh' => melodyschool_storage_get_array('sc_team_data', 'css_wh'),
			'position' => $position,
			'link' => $link,
			'email' => $email,
			'photo' => $photo,
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_team_item', $atts, $content);
	}
	add_shortcode('trx_team_item', 'melodyschool_sc_team_item');
}
// ---------------------------------- [/trx_team] ---------------------------------------
...
if (!function_exists('melodyschool_team_reg_shortcodes')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list',	'melodyschool_team_reg_shortcodes');
	function melodyschool_team_reg_shortcodes() {
		if (melodyschool_storage_isset('shortcodes')) {

			$users = melodyschool_get_list_users();
			$members = melodyschool_get_list_posts(false, array(
				'post_type'=>'team',
				'orderby'=>'title',
				'order'=>'asc',
				'return'=>'title'
			...
			$team_groups = melodyschool_get_list_terms(false, 'team_group');
			$team_styles = melodyschool_get_list_templates('team');
			$controls	 = melodyschool_get_list_slider_controls();

			melodyschool_sc_map_after('trx_tabs', array(
				...
				"trx_team" => array(
					...
					"params" => array(
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $team_groups)
						),
						"count" => array(
						...
						"orderby" => array(
							...
							"value" => "title",
							"type" => "select",
							"options" => melodyschool_get_sc_param('sorting')
						),
						"order" => array(
...
if (!function_exists('melodyschool_team_reg_shortcodes_vc')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list_vc',	'melodyschool_team_reg_shortcodes_vc');
	function melodyschool_team_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"class" => "",
						"std" => "no",
						"value" => array_flip(melodyschool_get_sc_param('yes_no')),
						"type" => "dropdown"
					),
					array(
						"param_name" => "controls",
					...
					array(
						...
						"class" => "",
						"value" => array_flip(melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $team_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
					...
					array(
						...
						"std" => "title",
						"class" => "",
						"value" => array_flip(melodyschool_get_sc_param('sorting')),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
		...
		class WPBakeryShortCode_Trx_Team extends MELODYSCHOOL_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Team_Item extends MELODYSCHOOL_VC_ShortCodeCollection {}
