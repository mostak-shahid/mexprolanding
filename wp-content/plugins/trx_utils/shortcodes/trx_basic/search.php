...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_search_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_search_theme_setup' );
	function melodyschool_sc_search_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_search_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_search_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_search')) {	
	function melodyschool_sc_search($atts, $content=null){	
		...
		if (empty($ajax)) $ajax = melodyschool_get_theme_option('use_ajax_search');
		if (empty($style)) $style = melodyschool_param_is_on(melodyschool_get_theme_option('fullscreen_search')) ? 'fullscreen' : 'regular';
		if ($style == 'fullscreen') {
			$ajax = "no";
			$state = "closed";
		}
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="search_wrap search_style_'.esc_attr($style).' search_state_'.esc_attr($state)
						. (melodyschool_param_is_on($ajax) ? ' search_ajax' : '')
					...
					. '>
						...
						. (melodyschool_param_is_on($ajax) ? '<div class="search_results widget_area' . ($scheme && !melodyschool_param_is_off($scheme) && !melodyschool_param_is_inherit($scheme) ? ' scheme_'.esc_attr($scheme) : '') . '"><a class="search_results_close icon-cancel"></a><div class="search_results_content"></div></div>' : '')
					. '</div>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_search', $atts, $content);
	}
	add_shortcode('trx_search', 'melodyschool_sc_search');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_search_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_search_reg_shortcodes');
	function melodyschool_sc_search_reg_shortcodes() {
	
		melodyschool_sc_map("trx_search", array(
			"title" => esc_html__("Search", 'trx_utils'),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_search_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_search_reg_shortcodes_vc');
	function melodyschool_sc_search_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Search extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>