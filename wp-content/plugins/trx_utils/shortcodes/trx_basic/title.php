...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_title_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_title_theme_setup' );
	function melodyschool_sc_title_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_title_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_title_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_title')) {	
	function melodyschool_sc_title($atts, $content=null){	
		...
		$css .= melodyschool_get_css_dimensions_from_values($width)
			.($align && $align!='none' && !melodyschool_param_is_inherit($align) ? 'text-align:' . esc_attr($align) .';' : '')
		...
		$pic = $style!='iconed' 
			...
			: '<span class="sc_title_icon sc_title_icon_'.esc_attr($position).'  sc_title_icon_'.esc_attr($image_size).($icon!='' && $icon!='none' ? ' '.esc_attr($icon) : '').'"'.'>'
				...
				.(empty($picture) && $image && $image!='none' ? '<img src="'.esc_url(melodyschool_strpos($image, 'http:')!==false ? $image : melodyschool_get_file_url('images/icons/'.($image).'.png')).'" alt="" />' : '')
				.'</span>';
		$output = '<h' . esc_attr($type) . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_title sc_title_'.esc_attr($style)
					.($align && $align!='none' && !melodyschool_param_is_inherit($align) ? ' sc_align_' . esc_attr($align) : '')
				...
				. '</h' . esc_attr($type) . '>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_title', $atts, $content);
	}
	add_shortcode('trx_title', 'melodyschool_sc_title');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_title_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_title_reg_shortcodes');
	function melodyschool_sc_title_reg_shortcodes() {
	
		melodyschool_sc_map("trx_title", array(
			...
			"params" => array(
				...
				"image" => array(
					...
					"type" => "images",
					"size" => "small",
					"options" => melodyschool_get_sc_param('images')
				),
				"picture" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_title_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_title_reg_shortcodes_vc');
	function melodyschool_sc_title_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"value" => melodyschool_get_sc_param('images'),
					"type" => "dropdown"
				),
				array(
					"param_name" => "picture",
		...
		class WPBakeryShortCode_Trx_Title extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>