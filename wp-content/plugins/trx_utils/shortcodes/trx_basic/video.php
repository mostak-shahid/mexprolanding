...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_video_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_video_theme_setup' );
	function melodyschool_sc_video_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_video_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_video_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_video')) {	
	function melodyschool_sc_video($atts, $content = null) {
		...
		if (empty($height) && empty($width)) {
			$width='100%';
			if (melodyschool_param_is_off(melodyschool_get_custom_option('substitute_video'))) $height="400";
		}
		$ed = melodyschool_substr($width, -1);
		...
		$url = $src!='' ? $src : $url;
		if ($image!='' && melodyschool_param_is_off($image))
			$image = '';
		else {
			if (melodyschool_param_is_on($autoplay) && is_singular() && !melodyschool_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if ($bg_image) {
					...
					if (!is_single() || !empty($image)) $image = melodyschool_get_resized_image_url(empty($image) ? get_the_ID() : $image, $thumb_sizes['w'], $thumb_sizes['h'], null, false, false, false);
				} else
					if (!is_single() || !empty($image)) $image = melodyschool_get_resized_image_url(empty($image) ? get_the_ID() : $image, $ed!='%' ? $width : null, $height);
				if (empty($image) && (!is_singular() || melodyschool_storage_get('blog_streampage')))	// || melodyschool_param_is_off($autoplay)))
					$image = melodyschool_get_video_cover_image($url);
			}
		}
		if ($bg_image > 0) {
		...
		$url = melodyschool_get_video_player_url($src!='' ? $src : $url);

		$autoplay_flag = empty($image);


        $autoplay_flag = $autoplay_flag && ($image && melodyschool_param_is_on(melodyschool_get_custom_option('substitute_video')));
        $autoplay_flag = $autoplay_flag && (melodyschool_param_is_on($autoplay) && is_singular() && !melodyschool_storage_get('blog_streampage'));
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. ($autoplay_flag ? ' autoplay="autoplay"' : '')
			. ' controls="controls" loop="loop"'
			. '>'
			. '</video>';
		if (melodyschool_param_is_off(melodyschool_get_custom_option('substitute_video'))) {
			if (melodyschool_param_is_on($frame)) $video = melodyschool_get_video_frame($video, $image, $css, $css_bg);
		} else {
		...
		return apply_filters('melodyschool_shortcode_output', $video, 'trx_video', $atts, $content);
	}
	add_shortcode("trx_video", "melodyschool_sc_video");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_video_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_video_reg_shortcodes');
	function melodyschool_sc_video_reg_shortcodes() {
	
		melodyschool_sc_map("trx_video", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_video_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_video_reg_shortcodes_vc');
	function melodyschool_sc_video_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Video extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>