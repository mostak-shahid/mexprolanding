<?php
/**
 * MelodySchool Framework: Services support
 *
...
// Theme init
if (!function_exists('melodyschool_services_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_services_theme_setup',1 );
	function melodyschool_services_theme_setup() {
		
		// Detect current page type, taxonomy and title (for custom post_types use priority < 10 to fire it handles early, than for standard post types)
		add_filter('melodyschool_filter_get_blog_type',			'melodyschool_services_get_blog_type', 9, 2);
		add_filter('melodyschool_filter_get_blog_title',		'melodyschool_services_get_blog_title', 9, 2);
		add_filter('melodyschool_filter_get_current_taxonomy',	'melodyschool_services_get_current_taxonomy', 9, 2);
		add_filter('melodyschool_filter_is_taxonomy',			'melodyschool_services_is_taxonomy', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_title',	'melodyschool_services_get_stream_page_title', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_link',	'melodyschool_services_get_stream_page_link', 9, 2);
		add_filter('melodyschool_filter_get_stream_page_id',	'melodyschool_services_get_stream_page_id', 9, 2);
		add_filter('melodyschool_filter_query_add_filters',		'melodyschool_services_query_add_filters', 9, 2);
		add_filter('melodyschool_filter_detect_inheritance_key','melodyschool_services_detect_inheritance_key', 9, 1);

		// Extra column for services lists
		if (melodyschool_get_theme_option('show_overriden_posts')=='yes') {
			add_filter('manage_edit-services_columns',			'melodyschool_post_add_options_column', 9);
			add_filter('manage_services_posts_custom_column',	'melodyschool_post_fill_options_column', 9, 2);
		}

		// Register shortcodes [trx_services] and [trx_services_item]
		add_action('melodyschool_action_shortcodes_list',		'melodyschool_services_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_services_reg_shortcodes_vc');
		
		// Add supported data types
		melodyschool_theme_support_pt('services');
		melodyschool_theme_support_tx('services_group');
	}
}

if ( !function_exists( 'melodyschool_services_settings_theme_setup2' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_services_settings_theme_setup2', 3 );
	function melodyschool_services_settings_theme_setup2() {
		// Add post type 'services' and taxonomy 'services_group' into theme inheritance list
		melodyschool_add_theme_inheritance( array('services' => array(
...
if ( !function_exists( 'melodyschool_is_services_page' ) ) {
	function melodyschool_is_services_page() {
		$is = in_array(melodyschool_storage_get('page_template'), array('blog-services', 'single-service'));
		if (!$is) {
			if (!melodyschool_storage_empty('pre_query'))
				$is = melodyschool_storage_call_obj_method('pre_query', 'get', 'post_type')=='services' 
						|| melodyschool_storage_call_obj_method('pre_query', 'is_tax', 'services_group') 
						|| (melodyschool_storage_call_obj_method('pre_query', 'is_page') 
								&& ($id=melodyschool_get_template_page_id('blog-services')) > 0 
				...
				$is = get_query_var('post_type')=='services' 
						...
						|| (is_page() && ($id=melodyschool_get_template_page_id('blog-services')) > 0 && $id==get_the_ID());
		}
		return $is;
	}
}
...
if ( !function_exists( 'melodyschool_services_detect_inheritance_key' ) ) {
	//Handler of add_filter('melodyschool_filter_detect_inheritance_key',	'melodyschool_services_detect_inheritance_key', 9, 1);
	function melodyschool_services_detect_inheritance_key($key) {
		if (!empty($key)) return $key;
		return melodyschool_is_services_page() ? 'services' : '';
	}
}

// Filter to detect current page slug
if ( !function_exists( 'melodyschool_services_get_blog_type' ) ) {
	//Handler of add_filter('melodyschool_filter_get_blog_type',	'melodyschool_services_get_blog_type', 9, 2);
	function melodyschool_services_get_blog_type($page, $query=null) {
...
if ( !function_exists( 'melodyschool_services_get_blog_title' ) ) {
	//Handler of add_filter('melodyschool_filter_get_blog_title',	'melodyschool_services_get_blog_title', 9, 2);
	function melodyschool_services_get_blog_title($title, $page) {
		if (!empty($title)) return $title;
		if ( melodyschool_strpos($page, 'services')!==false ) {
...
if ( !function_exists( 'melodyschool_services_get_stream_page_title' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_title',	'melodyschool_services_get_stream_page_title', 9, 2);
	function melodyschool_services_get_stream_page_title($title, $page) {
		if (!empty($title)) return $title;
		if (melodyschool_strpos($page, 'services')!==false) {
			if (($page_id = melodyschool_services_get_stream_page_id(0, $page=='services' ? 'blog-services' : $page)) > 0)
...
if ( !function_exists( 'melodyschool_services_get_stream_page_id' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_id',	'melodyschool_services_get_stream_page_id', 9, 2);
	function melodyschool_services_get_stream_page_id($id, $page) {
		if (!empty($id)) return $id;
		if (melodyschool_strpos($page, 'services')!==false) $id = melodyschool_get_template_page_id('blog-services');
		return $id;
	}
}

// Filter to detect stream page URL
if ( !function_exists( 'melodyschool_services_get_stream_page_link' ) ) {
	//Handler of add_filter('melodyschool_filter_get_stream_page_link',	'melodyschool_services_get_stream_page_link', 9, 2);
	function melodyschool_services_get_stream_page_link($url, $page) {
		if (!empty($url)) return $url;
		if (melodyschool_strpos($page, 'services')!==false) {
			$id = melodyschool_get_template_page_id('blog-services');
...
if ( !function_exists( 'melodyschool_services_get_current_taxonomy' ) ) {
	//Handler of add_filter('melodyschool_filter_get_current_taxonomy',	'melodyschool_services_get_current_taxonomy', 9, 2);
	function melodyschool_services_get_current_taxonomy($tax, $page) {
...
if ( !function_exists( 'melodyschool_services_is_taxonomy' ) ) {
	//Handler of add_filter('melodyschool_filter_is_taxonomy',	'melodyschool_services_is_taxonomy', 9, 2);
	function melodyschool_services_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			return $tax;
		else 
...
if ( !function_exists( 'melodyschool_services_query_add_filters' ) ) {
	//Handler of add_filter('melodyschool_filter_query_add_filters',	'melodyschool_services_query_add_filters', 9, 2);
	function melodyschool_services_query_add_filters($args, $filter) {
		if ($filter == 'services') {
...
[/trx_services]
*/
if ( !function_exists( 'melodyschool_sc_services' ) ) {
	function melodyschool_sc_services($atts, $content=null){	
		...
		if (melodyschool_param_is_off($slider) && $columns > 1 && $style == 'services-5' && !empty($image)) $columns = 2;
		if (!empty($image)) {
			if ($image > 0) {
		...
		melodyschool_storage_set('sc_services_data', array(
			'id' => $id,
            'style' => $style,
            'type' => $type,
            'columns' => $columns,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '') 
					...
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						...
						. (!melodyschool_param_is_off($equalheight) ? ' data-equal-height=".sc_services_item"' : '')
						. (!melodyschool_param_is_off($animation) ? ' data-animation="'.esc_attr(melodyschool_get_animation_classes($animation)).'"' : '')
					. '>'
					. (!empty($subtitle) ? '<h6 class="sc_services_subtitle sc_item_subtitle">' . trim(melodyschool_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_services_title sc_item_title">' . trim(melodyschool_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_services_descr sc_item_descr">' . trim(melodyschool_strmacros($description)) . '</div>' : '')
		...
		if (melodyschool_param_is_on($custom) && $content) {
			...
			$args = melodyschool_query_add_posts_and_cats($args, $ids, 'services', $cat, 'services_group');
			...
			while ( $query->have_posts() ) { 
				...
				$output .= melodyschool_show_post_layout($args);
			}
			wp_reset_postdata();
		}
		...
		$output .=  (!empty($link) ? '<div class="sc_services_button sc_item_button">'.melodyschool_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
					. '</div><!-- /.sc_services -->'
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_services', $atts, $content);
	}
	add_shortcode('trx_services', 'melodyschool_sc_services');
}


if ( !function_exists( 'melodyschool_sc_services_item' ) ) {
	function melodyschool_sc_services_item($atts, $content=null) {
		if (melodyschool_in_shortcode_blogger()) return '';
		extract(melodyschool_html_decode(shortcode_atts( array(
			// Individual params
			"icon" => "",
		...
		melodyschool_storage_inc_array('sc_services_data', 'counter');

		$id = $id ? $id : (melodyschool_storage_get_array('sc_services_data', 'id') ? melodyschool_storage_get_array('sc_services_data', 'id') . '_' . melodyschool_storage_get_array('sc_services_data', 'counter') : '');

		$descr = trim(chop(do_shortcode($content)));
		$readmore = $readmore=='(none)' ? melodyschool_storage_get_array('sc_services_data', 'readmore') : $readmore;

		$type = melodyschool_storage_get_array('sc_services_data', 'type');
		if (!empty($icon)) {
			$type = 'icons';
		} else if (!empty($image)) {
			$type = 'images';
			...
			$thumb_sizes = melodyschool_get_thumb_sizes(array('layout' => melodyschool_storage_get_array('sc_services_data', 'style')));
		...
		$args = array(
			'layout' => melodyschool_storage_get_array('sc_services_data', 'style'),
			'number' => melodyschool_storage_get_array('sc_services_data', 'counter'),
			'columns_count' => melodyschool_storage_get_array('sc_services_data', 'columns'),
			'slider' => melodyschool_storage_get_array('sc_services_data', 'slider'),
			'show' => false,
			...
			'tag_css' => $css,
			'tag_css_wh' => melodyschool_storage_get_array('sc_services_data', 'css_wh')
		);
		$output = melodyschool_show_post_layout($args, $post_data);
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_services_item', $atts, $content);
	}
	add_shortcode('trx_services_item', 'melodyschool_sc_services_item');
}
// ---------------------------------- [/trx_services] ---------------------------------------
...
if (!function_exists('melodyschool_services_reg_shortcodes')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list',	'melodyschool_services_reg_shortcodes');
	function melodyschool_services_reg_shortcodes() {
		if (melodyschool_storage_isset('shortcodes')) {

			$services_groups = melodyschool_get_list_terms(false, 'services_group');
			$services_styles = melodyschool_get_list_templates('services');
			$controls 		 = melodyschool_get_list_slider_controls();

			melodyschool_sc_map_after('trx_section', array(
				...
				"trx_services" => array(
					...
					"params" => array(
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $services_groups)
						),
						"count" => array(
...
if (!function_exists('melodyschool_services_reg_shortcodes_vc')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list_vc',	'melodyschool_services_reg_shortcodes_vc');
	function melodyschool_services_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"class" => "",
						"value" => array_flip(melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $services_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
		...
		class WPBakeryShortCode_Trx_Services extends MELODYSCHOOL_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Services_Item extends MELODYSCHOOL_VC_ShortCodeCollection {}
