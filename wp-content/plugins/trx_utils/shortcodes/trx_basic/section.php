...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_section_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_section_theme_setup' );
	function melodyschool_sc_section_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_section_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_section_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_section')) {	
	function melodyschool_sc_section($atts, $content=null){	
		...
		$css .= ($color !== '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($bg_image !== '' ? 'background-image:url(' . esc_url($bg_image) . ');'.(melodyschool_param_is_on($bg_tile) ? 'background-repeat:repeat;' : 'background-repeat:no-repeat;background-size:cover;') : '')
			.(!melodyschool_param_is_off($pan) ? 'position:relative;' : '')
			.($font_size != '' ? 'font-size:' . esc_attr(melodyschool_prepare_css_value($font_size)) . '; line-height: 1.3em;' : '')
			.($font_weight != '' && !melodyschool_param_is_inherit($font_weight) ? 'font-weight:' . esc_attr($font_weight) . ';' : '');
		$css_dim = melodyschool_get_css_dimensions_from_values($width, $height);
		if ($bg_image == '' && $bg_color == '' && $bg_overlay==0 && $bg_texture==0 && melodyschool_strlen($bg_texture)<2) $css .= $css_dim;
		
		$width  = melodyschool_prepare_css_value($width);
		$height = melodyschool_prepare_css_value($height);
	
		if ((!melodyschool_param_is_off($scroll) || !melodyschool_param_is_off($pan)) && empty($id)) $id = 'sc_section_'.str_replace('.', '', mt_rand());
	
		if (!melodyschool_param_is_off($scroll)) melodyschool_enqueue_slider();
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_section' 
					...
					. (melodyschool_param_is_on($scroll) && !melodyschool_param_is_off($scroll_controls) ? ' sc_scroll_controls sc_scroll_controls_'.esc_attr($scroll_dir).' sc_scroll_controls_type_'.esc_attr($scroll_controls) : '')
					. '"'
				...
				. '<div class="sc_section_inner">'
					. ($bg_image !== '' || $bg_color !== '' || $bg_overlay>0 || $bg_texture>0 || melodyschool_strlen($bg_texture)>2
						? '<div class="sc_section_overlay'.($bg_texture>0 ? ' texture_bg_'.esc_attr($bg_texture) : '') . '"'
							. ' style="' . ($bg_overlay>0 ? 'background-color:rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.min(1, max(0, $bg_overlay)).');' : '')
								...
								. '>'
								. '<div class="sc_section_content' . (melodyschool_param_is_on($bg_padding) ? ' padding_on' : ' padding_off') . '"'
					...
					. (melodyschool_param_is_on($scroll) 
						...
						: '')
					. (melodyschool_param_is_on($pan) 
						? '<div id="'.esc_attr($id).'_pan" class="sc_pan sc_pan_'.esc_attr($scroll_dir).'">' 
						: '')
                            . (!empty($title) ? '<h1 class="sc_section_title sc_item_title">' . trim(melodyschool_strmacros($title)) . '</h1>' : '')
                            . (!empty($subtitle) ? '<h6 class="sc_section_subtitle sc_item_subtitle">' . trim(melodyschool_strmacros($subtitle)) . '</h6>' : '')
							. (!empty($description) ? '<div class="sc_section_descr sc_item_descr">' . trim(melodyschool_strmacros($description)) . '</div>' : '')
							. '<div class="sc_section_content_wrap">' . do_shortcode($content) . '</div>'
							. (!empty($link) ? '<div class="sc_section_button sc_item_button">'.melodyschool_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
					. (melodyschool_param_is_on($pan) ? '</div>' : '')
					. (melodyschool_param_is_on($scroll) 
						? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($scroll_dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
							. (!melodyschool_param_is_off($scroll_controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
						: '')
					. ($bg_image !== '' || $bg_color !== '' || $bg_overlay > 0 || $bg_texture>0 || melodyschool_strlen($bg_texture)>2 ? '</div></div>' : '')
					. '</div>'
				. '</div>';
		if (melodyschool_param_is_on($dedicated)) {
			if (melodyschool_storage_get('sc_section_dedicated')=='') {
				melodyschool_storage_set('sc_section_dedicated', $output);
			}
			$output = '';
		}
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_section', $atts, $content);
	}
	add_shortcode('trx_section', 'melodyschool_sc_section');
}

if (!function_exists('melodyschool_sc_block')) {	
	function melodyschool_sc_block($atts, $content=null) {
        if (empty($atts)) $atts = array();
		$atts['class'] = (!empty($atts['class']) ? ' ' : '') . 'sc_section_block';
		return apply_filters('melodyschool_shortcode_output', melodyschool_sc_section($atts, $content), 'trx_block', $atts, $content);
	}
	add_shortcode('trx_block', 'melodyschool_sc_block');
}


/* Register shortcode in the internal SC Builder
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_section_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_section_reg_shortcodes');
	function melodyschool_sc_section_reg_shortcodes() {
		...
		$sc = array(
			...
			"params" => array(
				...
				"columns" => array(
					...
					"value" => "none",
					"type" => "checklist",
					"options" => melodyschool_get_sc_param('columns')
				), 
				"pan" => array(
				...
				"scroll_controls" => array(
					...
					"value" => "hide",
					"type" => "checklist",
					"options" => melodyschool_get_sc_param('controls')
				),
				"scheme" => array(
		...
		melodyschool_sc_map("trx_block", $sc);
		...
		melodyschool_sc_map("trx_section", $sc);
	}
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_section_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_section_reg_shortcodes_vc');
	function melodyschool_sc_section_reg_shortcodes_vc() {
		...
		$sc = array(
			...
			"params" => array(
				...
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(melodyschool_get_sc_param('columns')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "title",
				...
				array(
					...
					"value" => array_flip(melodyschool_get_sc_param('controls')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
		...
		class WPBakeryShortCode_Trx_Block extends MELODYSCHOOL_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Section extends MELODYSCHOOL_VC_ShortCodeCollection {}
	}
}
?>