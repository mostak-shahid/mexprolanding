<?php
/**
 * MelodySchool Framework: Testimonial support
 *
...
// Theme init
if (!function_exists('melodyschool_testimonial_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_testimonial_theme_setup', 1 );
	function melodyschool_testimonial_theme_setup() {
	
		// Add item in the admin menu
		add_filter('trx_utils_filter_override_options',		'melodyschool_testimonial_add_override_options');

		// Save data from override options
		add_action('save_post',				'melodyschool_testimonial_save_data');

		// Register shortcodes [trx_testimonials] and [trx_testimonials_item]
		add_action('melodyschool_action_shortcodes_list',		'melodyschool_testimonials_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_testimonials_reg_shortcodes_vc');

		// Meta box fields
		melodyschool_storage_set('testimonial_override_options', array(
		...
		melodyschool_theme_support_pt('testimonial');
		melodyschool_theme_support_tx('testimonial_group');
...
if (!function_exists('melodyschool_testimonial_add_override_options')) {
    //add_action('trx_utils_filter_override_options', 'melodyschool_testimonial_add_override_options');
    function melodyschool_testimonial_add_override_options($boxes = array()) {
        $boxes[] = array_merge(melodyschool_storage_get('testimonial_override_options'), array('callback' => 'melodyschool_testimonial_show_override_options'));
        return $boxes;
    }
}
...
if (!function_exists('melodyschool_testimonial_show_override_options')) {
	function melodyschool_testimonial_show_override_options() {
		global $post;

		// Use nonce for verification
		echo '<input type="hidden" name="override_options_testimonial_nonce" value="'.esc_attr(wp_create_nonce(admin_url())).'" />';
		
		$data = get_post_meta($post->ID, melodyschool_storage_get('options_prefix').'_testimonial_data', true);
	
		$fields = melodyschool_storage_get_array('testimonial_override_options', 'fields');
		?>
...
// Save data from override options
if (!function_exists('melodyschool_testimonial_save_data')) {
	//Handler of add_action('save_post', 'melodyschool_testimonial_save_data');
	function melodyschool_testimonial_save_data($post_id) {
		// verify nonce
		if ( !wp_verify_nonce( melodyschool_get_value_gp('override_options_testimonial_nonce'), admin_url() ) )
			return $post_id;
		...
		update_post_meta($post_id, melodyschool_storage_get('options_prefix').'_testimonial_data', $data);
	}
}
...
if (!function_exists('melodyschool_sc_testimonials')) {
	function melodyschool_sc_testimonials($atts, $content=null){	
		...
		if ($bg_overlay > 0) {
			if ($bg_color=='') $bg_color = melodyschool_get_scheme_color('bg');
			$rgb = melodyschool_hex2rgb($bg_color);
		}
		...
		melodyschool_storage_set('sc_testimonials_data', array(
			'id' => $id,
            'style' => $style,
            'columns' => $columns,
            'counter' => 0,
		...
		$output = ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || melodyschool_strlen($bg_texture)>2 || ($scheme && !melodyschool_param_is_off($scheme) && !melodyschool_param_is_inherit($scheme))
					? '<div class="sc_testimonials_wrap sc_section'
						...
						. '<div class="sc_section_overlay'.($bg_texture>0 ? ' texture_bg_'.esc_attr($bg_texture) : '') . '"'
								. ' style="' . ($bg_overlay>0 ? 'background-color:rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.min(1, max(0, $bg_overlay)).');' : '')
									. (melodyschool_strlen($bg_texture)>2 ? 'background-image:url('.esc_url($bg_texture).');' : '')
									. '"'
				...
				. ' class="sc_testimonials sc_testimonials_style_'.esc_attr($style)
					...
					. '"'
				. ($bg_color=='' && $bg_image=='' && $bg_overlay==0 && ($bg_texture=='' || $bg_texture=='0') && !melodyschool_param_is_off($animation) ? ' data-animation="'.esc_attr(melodyschool_get_animation_classes($animation)).'"' : '')
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. '>'
			. (!empty($subtitle) ? '<h6 class="sc_testimonials_subtitle sc_item_subtitle">' . trim(melodyschool_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_testimonials_title sc_item_title">' . trim(melodyschool_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_testimonials_descr sc_item_descr">' . trim(melodyschool_strmacros($description)) . '</div>' : '')
		...
		if (melodyschool_param_is_on($custom) && $content) {
			...
			$args = melodyschool_query_add_posts_and_cats($args, $ids, 'testimonial', $cat, 'testimonial_group');
			...
			while ( $query->have_posts() ) { 
				...
				$post_meta = get_post_meta($post_data['post_id'], melodyschool_storage_get('options_prefix').'_testimonial_data', true);
		...
		$output .= '</div>'
					. ($bg_color!='' || $bg_image!='' || $bg_overlay>0 || $bg_texture>0 || melodyschool_strlen($bg_texture)>2 || ($scheme && !melodyschool_param_is_off($scheme) && !melodyschool_param_is_inherit($scheme))
						?  '</div></div>'
						: '');
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_testimonials', $atts, $content);
	}
	add_shortcode('trx_testimonials', 'melodyschool_sc_testimonials');
}
	
	
if (!function_exists('melodyschool_sc_testimonials_item')) {
	function melodyschool_sc_testimonials_item($atts, $content=null){	
		if (melodyschool_in_shortcode_blogger()) return '';
		extract(melodyschool_html_decode(shortcode_atts(array(
			// Individual params
			"author" => "",
		...
		melodyschool_storage_inc_array('sc_testimonials_data', 'counter');
	
		$id = $id ? $id : (melodyschool_storage_get_array('sc_testimonials_data', 'id') ? melodyschool_storage_get_array('sc_testimonials_data', 'id') . '_' . melodyschool_storage_get_array('sc_testimonials_data', 'counter') : '');
	
		$thumb_sizes = melodyschool_get_thumb_sizes(array('layout' => melodyschool_storage_get_array('sc_testimonials_data', 'style')));
		...
		$args = array(
			'layout' => melodyschool_storage_get_array('sc_testimonials_data', 'style'),
			'number' => melodyschool_storage_get_array('sc_testimonials_data', 'counter'),
			'columns_count' => melodyschool_storage_get_array('sc_testimonials_data', 'columns'),
			'slider' => melodyschool_storage_get_array('sc_testimonials_data', 'slider'),
			'show' => false,
			'descr'  => 0,
			'tag_id' => $id,
			'tag_class' => $class,
			'tag_animation' => '',
			'tag_css' => $css,
			'tag_css_wh' => melodyschool_storage_get_array('sc_testimonials_data', 'css_wh'),
			'author' => $author,
			'position' => $position,
			'link' => $link,
			'email' => $email,
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_testimonials_item', $atts, $content);
	}
	add_shortcode('trx_testimonials_item', 'melodyschool_sc_testimonials_item');
}
// ---------------------------------- [/trx_testimonials] ---------------------------------------
...
if (!function_exists('melodyschool_testimonials_reg_shortcodes')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list',	'melodyschool_testimonials_reg_shortcodes');
	function melodyschool_testimonials_reg_shortcodes() {
		if (melodyschool_storage_isset('shortcodes')) {

			$testimonials_groups = melodyschool_get_list_terms(false, 'testimonial_group');
			$testimonials_styles = melodyschool_get_list_templates('testimonials');
			$controls = melodyschool_get_list_slider_controls();

			melodyschool_sc_map_before('trx_title', array(
				...
				"trx_testimonials" => array(
					...
					"params" => array(
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $testimonials_groups)
						),
						"count" => array(
...
if (!function_exists('melodyschool_testimonials_reg_shortcodes_vc')) {
	//Handler of add_filter('melodyschool_action_shortcodes_list_vc',	'melodyschool_testimonials_reg_shortcodes_vc');
	function melodyschool_testimonials_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"params" => array(
                    ...
                    array(
                        ...
                        "description" => wp_kses_data( __("Select testimonials style color", 'trx_utils') ),
                        "class" => "",
                        "value" => array(
                            esc_html__('Original', 'trx_utils') => 'style_color_original',
                            esc_html__('Transparent', 'trx_utils') => 'no_image'
                        ),
                        "type" => "dropdown"
                    ),
					array(
					...
					array(
						...
						"class" => "",
						"value" => array_flip(melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $testimonials_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
		...
		class WPBakeryShortCode_Trx_Testimonials extends MELODYSCHOOL_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Testimonials_Item extends MELODYSCHOOL_VC_ShortCodeCollection {}
