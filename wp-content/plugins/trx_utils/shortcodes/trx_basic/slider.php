...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_slider_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_slider_theme_setup' );
	function melodyschool_sc_slider_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_slider_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_slider_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_slider')) {	
	function melodyschool_sc_slider($atts, $content=null){	
		...
		extract(melodyschool_html_decode(shortcode_atts(array(
			...
			"slides_per_view" => 1,
			"titles" => "no",
			"descriptions" => melodyschool_get_custom_option('slider_info_descriptions'),
			"links" => "no",
			"align" => "",
			"interval" => "",
			"date_format" => "",
		...
		if (!empty($height) && melodyschool_param_is_on($autoheight)) $autoheight = "off";
		...
		melodyschool_storage_set('sc_slider_data', array(
			'engine' => $engine,
            'width'  => melodyschool_prepare_css_value($width),
            'height' => melodyschool_prepare_css_value($height),
            'links'  => melodyschool_param_is_on($links),
            'bg_image' => melodyschool_get_theme_setting('slides_type')=='bg',
            'crop_image' => $crop
            )
        );
		...
		$class2 = melodyschool_get_css_position_as_classes($top, $right, $bottom, $left);
		...
		$output = (in_array($pagination, array('full', 'over')) 
				...
				. ' class="sc_slider sc_slider_' . esc_attr($engine)
					...
					. (melodyschool_param_is_on($controls) ? ' sc_slider_controls' : ' sc_slider_nocontrols')
					. (melodyschool_param_is_on($pagination) ? ' sc_slider_pagination' : ' sc_slider_nopagination')
					. (melodyschool_storage_get_array('sc_slider_data', 'bg_image') ? ' sc_slider_bg' : ' sc_slider_images')
					. (!in_array($pagination, array('full', 'over')) 
							? ($class2 ? ' '.esc_attr($class2) : '') . ($align!='' && $align!='none' ? ' align'.esc_attr($align) : '')
							: '')
					. '"'
				. (!in_array($pagination, array('full', 'over')) && !melodyschool_param_is_off($animation) ? ' data-animation="'.esc_attr(melodyschool_get_animation_classes($animation)).'"' : '')
		...
		if ($engine=='revo') {
			...
			$output .= '<div class="slides'
				...
				.($engine=='swiper' && melodyschool_storage_get_array('sc_slider_data', 'bg_image') ? ' style="'.esc_attr($hs).'"' : '')
				.'>';
			...
			if (melodyschool_param_is_on($custom) && $content) {
				...
				$args = melodyschool_query_add_filters($args, 'thumbs');
				$args = melodyschool_query_add_posts_and_cats($args, $ids, $post_type, $cat);
				...
				while ( $query->have_posts() ) { 
					...
					if (melodyschool_param_is_on($crop)) {
						$post_attachment = melodyschool_storage_get_array('sc_slider_data', 'bg_image')
							? melodyschool_get_resized_image_url($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null)
							: melodyschool_get_resized_image_tag($post_attachment, !empty($width) && (float) $width.' ' == $width.' ' ? $width : null, !empty($height) && (float) $height.' ' == $height.' ' ? $height : null);
					} else if (!melodyschool_storage_get_array('sc_slider_data', 'bg_image')) {
					...
					if (in_array($pagination, array('full', 'over'))) {
						$old_output = $output;
						$output = '';
						if (file_exists(melodyschool_get_file_dir('templates/_parts/widgets-posts.php'))) {
					...
					$output .= '<div' 
						...
						. ' style="'
							. (melodyschool_storage_get_array('sc_slider_data', 'bg_image') ? 'background-image:url(' . esc_url($post_attachment) . ');' : '') . ($ws) . ($hs)
							. '"'
						. '>' 
						. (melodyschool_param_is_on($links) ? '<a href="'.esc_url($post_link).'" title="'.esc_attr($post_title).'">' : '')
						. (!melodyschool_storage_get_array('sc_slider_data', 'bg_image') ? $post_attachment : '')
						;
					$caption = $engine=='swiper' ? '' : $caption;
					if (!melodyschool_param_is_off($titles)) {
						$post_hover_bg  = melodyschool_get_scheme_color('text_link');
						$post_bg = '';
						if ($post_hover_bg!='' && !melodyschool_is_inherit_option($post_hover_bg)) {
							$rgb = melodyschool_hex2rgb($post_hover_bg);
						...
						$post_descr = melodyschool_get_post_excerpt();
						if (melodyschool_get_custom_option("slider_info_category")=='yes') { // || empty($cat)) {
							// Get all post's categories
							$post_tax = melodyschool_get_taxonomy_categories_by_post_type($post_type);
							if (!empty($post_tax)) {
								$post_terms = melodyschool_get_terms_by_post_id(array('post_id'=>$post_id, 'taxonomy'=>$post_tax));
								if (!empty($post_terms[$post_tax])) {
									...
									if ($post_category!='') {
										$caption .= '<div class="sc_slider_category"'.(melodyschool_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.esc_attr($post_accent_color).'"' : '').'><a href="'.esc_url($post_category_link).'">'.($post_category).'</a></div>';
									}
								}
							}
						}
						$output_reviews = '';
						if (melodyschool_get_custom_option('show_reviews')=='yes' && melodyschool_get_custom_option('slider_info_reviews')=='yes') {
							$avg_author = melodyschool_reviews_marks_to_display(get_post_meta($post_id, melodyschool_storage_get('options_prefix').'_reviews_avg'.((melodyschool_get_theme_option('reviews_first')=='author' && $orderby != 'users_rating') || $orderby == 'author_rating' ? '' : '2'), true));
							if ($avg_author > 0) {
								$output_reviews .= '<div class="sc_slider_reviews post_rating reviews_summary blog_reviews' . (melodyschool_get_custom_option("slider_info_category")=='yes' ? ' after_category' : '') . '">'
									. '<div class="criteria_summary criteria_row">' . trim(melodyschool_reviews_get_summary_stars($avg_author, false, false, 5)) . '</div>'
									. '</div>';
							}
						}
						if (melodyschool_get_custom_option("slider_info_category")=='yes') $caption .= $output_reviews;
						$caption .= '<h3 class="sc_slider_subtitle"><a href="'.esc_url($post_link).'">'.($post_title).'</a></h3>';
						if (melodyschool_get_custom_option("slider_info_category")!='yes') $caption .= $output_reviews;
						if ($descriptions > 0) {
							$caption .= '<div class="sc_slider_descr">'.trim(melodyschool_strshort($post_descr, $descriptions)).'</div>';
						}
						$caption .= '</div>';
					}
					$output .= ($engine=='swiper' ? $caption : '') . (melodyschool_param_is_on($links) ? '</a>' : '' ) . '</div>';
				}
				wp_reset_postdata();
			}
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_slider', $atts, $content);
	}
	add_shortcode('trx_slider', 'melodyschool_sc_slider');
}


if (!function_exists('melodyschool_sc_slider_item')) {	
	function melodyschool_sc_slider_item($atts, $content=null) {
		if (melodyschool_in_shortcode_blogger()) return '';
		extract(melodyschool_html_decode(shortcode_atts( array(
			// Individual params
			"src" => "",
		...
		$engine = melodyschool_storage_get_array('sc_slider_data', 'engine');
		$crop_image = melodyschool_storage_get_array('sc_slider_data', 'crop_image');
		$bg_image = melodyschool_storage_get_array('sc_slider_data', 'bg_image');
		$links = melodyschool_storage_get_array('sc_slider_data', 'links');
		$width = melodyschool_storage_get_array('sc_slider_data', 'width');
		$height = melodyschool_storage_get_array('sc_slider_data', 'height');
	
		if ($src && melodyschool_param_is_on($crop_image)) {
			$src = melodyschool_storage_get_array('sc_slider_data', 'bg_image')
				? melodyschool_get_resized_image_url($src, !empty($width) && melodyschool_strpos($width, '%')===false ? $width : null, !empty($height) && melodyschool_strpos($height, '%')===false ? $height : null)
				: melodyschool_get_resized_image_tag($src, !empty($width) && melodyschool_strpos($width, '%')===false ? $width : null, !empty($height) && melodyschool_strpos($height, '%')===false ? $height : null);
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '').' class="'.esc_attr($engine).'-slide' . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
				. ($css ? ' style="'.esc_attr($css).'"' : '')
				.'>' 
				. ($src && melodyschool_param_is_on($links) ? '<a href="'.esc_url($src).'">' : '')
				. ($src && !$bg_image ? $src : $content)
				. ($src && melodyschool_param_is_on($links) ? '</a>' : '')
			. '</div>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_slider_item', $atts, $content);
	}
	add_shortcode('trx_slider_item', 'melodyschool_sc_slider_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_slider_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_slider_reg_shortcodes');
	function melodyschool_sc_slider_reg_shortcodes() {
	
		melodyschool_sc_map("trx_slider", array(
			...
			"params" => array_merge(array(
				"engine" => array(
					...
					"value" => "swiper",
					"type" => "checklist",
					"options" => melodyschool_get_sc_param('sliders')
				),
				"align" => array(
				...
				function_exists('melodyschool_exists_revslider') && melodyschool_exists_revslider() ? array(
				"alias" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => melodyschool_get_sc_param('revo_sliders')
				)) : array(), array(
				"cat" => array(
					...
					"style" => "list",
					"multiple" => true,
					"options" => melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), melodyschool_get_sc_param('categories'))
				),
				"count" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_slider_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_slider_reg_shortcodes_vc');
	function melodyschool_sc_slider_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array_merge(array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(melodyschool_get_sc_param('sliders')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
				...
				array(
					...
					"value" => array_flip(melodyschool_array_merge(array('none' => esc_html__('- Select slider -', 'trx_utils')), melodyschool_get_sc_param('revo_sliders'))),
					"type" => "dropdown"
				)) : array(), array(
				array(
					"param_name" => "cat",
					...
					"class" => "",
					"value" => array_flip(melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), melodyschool_get_sc_param('categories'))),
					"type" => "dropdown"
				),
				array(
					"param_name" => "count",
		...
		class WPBakeryShortCode_Trx_Slider extends MELODYSCHOOL_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Slider_Item extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>