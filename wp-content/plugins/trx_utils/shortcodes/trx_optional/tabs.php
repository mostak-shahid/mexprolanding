...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_tabs_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_tabs_theme_setup' );
	function melodyschool_sc_tabs_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_tabs_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_tabs')) {	
	function melodyschool_sc_tabs($atts, $content = null) {
		...
		melodyschool_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = melodyschool_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('melodyschool_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	add_shortcode("trx_tabs", "melodyschool_sc_tabs");
}


if (!function_exists('melodyschool_sc_tab')) {	
	function melodyschool_sc_tab($atts, $content = null) {
		if (melodyschool_in_shortcode_blogger()) return '';
		extract(melodyschool_html_decode(shortcode_atts(array(
			// Individual params
		...
		melodyschool_storage_inc_array('sc_tab_data', 'counter');
		$counter = melodyschool_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : melodyschool_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = melodyschool_storage_get_array('sc_tab_data', 'titles');
		...
		melodyschool_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (melodyschool_param_is_on(melodyschool_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(melodyschool_storage_get_array('sc_tab_data', 'height') != '' ? melodyschool_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (melodyschool_param_is_on(melodyschool_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	add_shortcode("trx_tab", "melodyschool_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_tabs_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_tabs_reg_shortcodes');
	function melodyschool_sc_tabs_reg_shortcodes() {
	
		melodyschool_sc_map("trx_tabs", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_tabs_reg_shortcodes_vc');
	function melodyschool_sc_tabs_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Tabs extends MELODYSCHOOL_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends MELODYSCHOOL_VC_ShortCodeTab {}
	}
}
?>