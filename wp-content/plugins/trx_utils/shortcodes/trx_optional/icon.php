...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_icon_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_icon_theme_setup' );
	function melodyschool_sc_icon_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_icon_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_icon_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_icon')) {	
	function melodyschool_sc_icon($atts, $content=null){	
		...
		$css2 = ($font_weight != '' && !melodyschool_is_inherit_option($font_weight) ? 'font-weight:'. esc_attr($font_weight).';' : '')
			. ($font_size != '' ? 'font-size:' . esc_attr(melodyschool_prepare_css_value($font_size)) . '; line-height: ' . (!$bg_shape || melodyschool_param_is_inherit($bg_shape) ? '1' : '1.2') . 'em;' : '')
		...
		$output = $icon!='' 
			? ($link ? '<a href="'.esc_url($link).'"' : '<span') . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_icon '.esc_attr($icon)
					. ($bg_shape && !melodyschool_param_is_inherit($bg_shape) ? ' sc_icon_shape_'.esc_attr($bg_shape) : '')
				...
				.($link ? '</a>' : '</span>')
			: '';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_icon', $atts, $content);
	}
	add_shortcode('trx_icon', 'melodyschool_sc_icon');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_icon_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_icon_reg_shortcodes');
	function melodyschool_sc_icon_reg_shortcodes() {
	
		melodyschool_sc_map("trx_icon", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_icon_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_icon_reg_shortcodes_vc');
	function melodyschool_sc_icon_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Icon extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>