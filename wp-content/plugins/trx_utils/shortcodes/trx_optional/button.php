...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('melodyschool_sc_button_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_sc_button_theme_setup' );
	function melodyschool_sc_button_theme_setup() {
		add_action('melodyschool_action_shortcodes_list', 		'melodyschool_sc_button_reg_shortcodes');
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_sc_button_reg_shortcodes_vc');
	}
}
...
if (!function_exists('melodyschool_sc_button')) {	
	function melodyschool_sc_button($atts, $content=null){	
		...
		if (melodyschool_param_is_on($popup)) melodyschool_enqueue_popup('magnific');
		$output = '<a href="' . (empty($link) ? '#' : $link) . '"'
			...
			. ' class="sc_button sc_button_' . esc_attr($type) 
					...
					. (melodyschool_param_is_on($popup) ? ' sc_popup_link' : '') 
					. '"'
			. ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. '</a>';
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_button', $atts, $content);
	}
	add_shortcode('trx_button', 'melodyschool_sc_button');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_button_reg_shortcodes' ) ) {
	//add_action('melodyschool_action_shortcodes_list', 'melodyschool_sc_button_reg_shortcodes');
	function melodyschool_sc_button_reg_shortcodes() {
	
		melodyschool_sc_map("trx_button", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'melodyschool_sc_button_reg_shortcodes_vc' ) ) {
	//add_action('melodyschool_action_shortcodes_list_vc', 'melodyschool_sc_button_reg_shortcodes_vc');
	function melodyschool_sc_button_reg_shortcodes_vc() {
			...
			"params" => array(
                ...
                array(
                    ...
                    "value" => array(
                        ...
                        esc_html__('Dark', 'trx_utils') => 'style_color_dark',
                        esc_html__('Dark with border hover', 'trx_utils') => 'style_color_dark_hover'
                    ),
                    "type" => "dropdown"
                ),
				array(
		...
		class WPBakeryShortCode_Trx_Button extends MELODYSCHOOL_VC_ShortCodeSingle {}
	}
}
?>