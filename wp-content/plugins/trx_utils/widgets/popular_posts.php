...
// Theme init
if (!function_exists('melodyschool_widget_popular_posts_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_widget_popular_posts_theme_setup', 1 );
	function melodyschool_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('melodyschool_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'melodyschool_widget_popular_posts_load' );
	function melodyschool_widget_popular_posts_load() {
		register_widget('melodyschool_widget_popular_posts');
	}
}

// Widget Class
class melodyschool_widget_popular_posts extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'melodyschool_widget_popular_posts', esc_html__('MelodySchool - Most Popular & Commented Posts', 'trx_utils'), $widget_ops );

		// Add thumb sizes into list
		melodyschool_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'trx_utils') ) );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? melodyschool_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = melodyschool_storage_get('options_prefix').'_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = melodyschool_storage_get('options_prefix').'_post_likes_count';
				$args['orderby'] = 'meta_value_num';
...
*/
if ( !function_exists( 'melodyschool_sc_widget_popular_posts' ) ) {
	function melodyschool_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'melodyschool_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (melodyschool_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, melodyschool_prepare_widgets_args(melodyschool_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	add_shortcode("trx_widget_popular_posts", "melodyschool_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('melodyschool_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_popular_posts_reg_shortcodes_vc');
	function melodyschool_widget_popular_posts_reg_shortcodes_vc() {
