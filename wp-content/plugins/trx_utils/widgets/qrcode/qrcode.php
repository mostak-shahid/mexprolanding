 ...
 */
add_action( 'widgets_init', 'melodyschool_widget_qrcode_load' );

/**
 * Register our widget.
 */
function melodyschool_widget_qrcode_load() {
	register_widget('melodyschool_widget_qrcode');
}

/**
 * QRCode Widget class.
 */
class melodyschool_widget_qrcode extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'melodyschool_widget_qrcode');

		/* Create the widget. */
		parent::__construct( 'melodyschool_widget_qrcode', esc_html__('MelodySchool - QRCode generator', 'trx_utils'), $widget_ops, $control_ops );
	...
	function form($instance) {
		...
		wp_enqueue_script('widget-qrcode-script', trx_utils_get_file_url('widgets/qrcode/jquery.qrcode-0.6.0.min.js'), array(), null, true);

		/* Set up some default widget settings. */
		$address = explode(',', melodyschool_get_theme_option('user_address'));
		$defaults = array(
			'title' => '', 
		...
		<div class="widget_qrcode">
        	<div class="qrcode_tabs">
                <ul class="tabs">
                    ...
                    <li><a href="#tab_fields" onmousedown="melodyschool_qrcode_init()"><?php esc_html_e('Personal Data', 'trx_utils'); ?></a></li>
                    <li><a href="#tab_text" onmousedown="melodyschool_qrcode_init()"><?php esc_html_e('Any Text', 'trx_utils'); ?></a></li>
                </ul>
                <div id="tab_settings" class="tab_content tab_settings">
                    <p>
                        ...
                        <input class="fld_title" onfocus="melodyschool_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($instance['title']); ?>" class="widgets_param_fullwidth" />
                    </p>
                    <p>
                        <label><?php esc_html_e('Show as QR Code:', 'trx_utils'); ?></label><br />
                        <input class="fld_show_what" onfocus="melodyschool_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="1" type="radio" <?php echo (1==$instance['show_what'] ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1"> <?php esc_html_e('Personal VCard', 'trx_utils'); ?></label>
                        <input class="fld_show_what" onfocus="melodyschool_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="0" type="radio" <?php echo (0==$instance['show_what'] ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0"> <?php esc_html_e('Any text', 'trx_utils'); ?></label>
                    </p>
                    <p>
                        <input class="fld_show_personal" onfocus="melodyschool_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_personal')); ?>" name="<?php echo esc_attr($this->get_field_name('show_personal')); ?>" value="1" type="checkbox" <?php echo (1==$instance['show_personal'] ? 'checked="checked"' : ''); ?> />
                    ...
                    <p>
                        ...
                        <input onmousedown="melodyschool_qrcode_init()" onfocus="melodyschool_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('width')); ?>" name="<?php echo esc_attr($this->get_field_name('width')); ?>" value="<?php echo esc_attr($instance['width']); ?>" class="widgets_param_fullwidth fld_width" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('color')); ?>"><?php esc_html_e('Color:', 'trx_utils'); ?></label>
                        <input onmousedown="melodyschool_qrcode_init()" onfocus="melodyschool_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('color')); ?>" name="<?php echo esc_attr($this->get_field_name('color')); ?>" value="<?php echo esc_attr($instance['color']); ?>" class="widgets_param_fullwidth iColorPicker fld_color" style="background-color:<?php echo esc_attr($instance['color']); ?>" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('bg')); ?>"><?php esc_html_e('Bg color:', 'trx_utils'); ?></label>
                        <input onmousedown="melodyschool_qrcode_init()" onfocus="melodyschool_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('bg')); ?>" name="<?php echo esc_attr($this->get_field_name('bg')); ?>" value="<?php echo esc_attr($instance['bg']); ?>" style="background-color:<?php echo esc_attr($instance['bg']); ?>" class="widgets_param_fullwidth iColorPicker fld_bg" />
                    </p>
                </div>
            ...
            <div id="<?php echo esc_attr($this->get_field_id('qrcode_image')); ?>" class="qrcode_image"><img src="<?php melodyschool_show_layout($instance['image']); ?>" alt="" /></div>
...
<?php if ($instance['show_personal']==1) { ?>
				...
				<?php if ($instance['show_what']==1) { ?>
					...
					?>
				<?php } else { ?>
                    <li class="text odd first"><?php melodyschool_show_layout($instance['text']); ?></li>
				<?php } ?>
                </ul>
<?php } ?>
            </div>
        ...
        <<?php echo esc_attr(melodyschool_storage_get('tag_open'));?>>
            jQuery(document).ready(function(){
				melodyschool_qrcode_init();
				melodyschool_color_picker();
            });
			function melodyschool_qrcode_init() {
				jQuery('#widgets-right .widget_qrcode:not(.inited)').each(function() {
					...
					widget.find('input.iColorPicker:not(.colored)').each(function() {
						...
						if (obj.attr('id').indexOf('__i__') < 0) {
							...
							melodyschool_set_color_picker(obj.attr('id'));
						}
					});
					widget.find('div.qrcode_tabs').tabs();
					widget.on('click', '.fld_button_draw', function() {
						melodyschool_qrcode_update(widget);
					});
            ...
            function melodyschool_qrcode_update(widget) {
				melodyschool_qrcode_show(widget, {
			...
			function melodyschool_qrcode_show(widget, vc, opt) {
				if (opt.show_what==1) {
			...
			if (!window.melodyschool_set_color_picker) {
				function melodyschool_set_color_picker(id_picker) {
