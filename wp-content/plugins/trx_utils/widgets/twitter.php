...
// Theme init
if (!function_exists('melodyschool_widget_twitter_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_widget_twitter_theme_setup', 1 );
	function melodyschool_widget_twitter_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_twitter_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('melodyschool_widget_twitter_load')) {
	add_action( 'widgets_init', 'melodyschool_widget_twitter_load' );
	function melodyschool_widget_twitter_load() {
		register_widget( 'melodyschool_widget_twitter' );
	}
}

// Widget Class
class melodyschool_widget_twitter extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'melodyschool_widget_twitter', esc_html__('MelodySchool - Twitter', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$twitter_username = !empty($instance['twitter_username']) ? $instance['twitter_username'] : melodyschool_get_theme_option('twitter_username');
		$twitter_consumer_key = !empty($instance['twitter_consumer_key']) ? $instance['twitter_consumer_key'] : melodyschool_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = !empty($instance['twitter_consumer_secret']) ? $instance['twitter_consumer_secret'] : melodyschool_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = !empty($instance['twitter_token_key']) ? $instance['twitter_token_key'] : melodyschool_get_theme_option('twitter_token_key');
		$twitter_token_secret = !empty($instance['twitter_token_secret']) ? $instance['twitter_token_secret'] : melodyschool_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, !empty($instance['twitter_count']) ? (int) $instance['twitter_count'] : (int) melodyschool_get_theme_option('twitter_count'));	
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $tweet) {
				...
				$output .= '<li class="theme_text' . ($cnt==$twitter_count-1 ? ' last' : '') . '">' . force_balance_tags(melodyschool_prepare_twitter_text($tweet)) . '<a href="' . esc_url('https://twitter.com/'.($twitter_username)) . '" class="username" target="_blank">@' . ($tweet['user']['screen_name']) . '</a></li>';
...
*/
if ( !function_exists( 'melodyschool_sc_widget_twitter' ) ) {
	function melodyschool_sc_widget_twitter($atts, $content=null){	
		...
		extract($atts);
		$type = 'melodyschool_widget_twitter';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			...
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_twitter' 
								. (melodyschool_exists_visual_composer() ? ' vc_widget_twitter wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, melodyschool_prepare_widgets_args(melodyschool_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_twitter', 'widget_twitter') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_widget_twitter', $atts, $content);
	}
	add_shortcode("trx_widget_twitter", "melodyschool_sc_widget_twitter");
}


// Add [trx_widget_twitter] in the VC shortcodes list
if (!function_exists('melodyschool_widget_twitter_reg_shortcodes_vc')) {
	function melodyschool_widget_twitter_reg_shortcodes_vc() {
