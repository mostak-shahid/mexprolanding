...
// Theme init
if (!function_exists('melodyschool_widget_recent_reviews_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_widget_recent_reviews_theme_setup', 1 );
	function melodyschool_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('melodyschool_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'melodyschool_widget_recent_reviews_load' );
	function melodyschool_widget_recent_reviews_load() {
		register_widget('melodyschool_widget_recent_reviews');
	}
}

// Widget Class
class melodyschool_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'trx_utils'));
		parent::__construct( 'melodyschool_widget_recent_reviews', esc_html__('MelodySchool - Recent reviews', 'trx_utils'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		if ($q->have_posts()) {
			...
			while ($q->have_posts()) { $q->the_post();
				...
				get_template_part(melodyschool_get_file_slug('templates/_parts/widget-posts.php'));
...
*/
if ( !function_exists( 'melodyschool_sc_widget_recent_reviews' ) ) {
	function melodyschool_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'melodyschool_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, melodyschool_prepare_widgets_args(melodyschool_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	add_shortcode("trx_widget_recent_reviews", "melodyschool_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('melodyschool_widget_recent_reviews_reg_shortcodes_vc')) {
	function melodyschool_widget_recent_reviews_reg_shortcodes_vc() {
