...
// Theme init
if (!function_exists('melodyschool_widget_recent_news_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_widget_recent_news_theme_setup', 1 );
	function melodyschool_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('melodyschool_widget_recent_news_load')) {
	add_action( 'widgets_init', 'melodyschool_widget_recent_news_load' );
	function melodyschool_widget_recent_news_load() {
		register_widget('melodyschool_widget_recent_news');
	}
}


// Widget Class
class melodyschool_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'trx_utils'));
		parent::__construct( 'melodyschool_widget_recent_news', esc_html__('MelodySchool - Recent News', 'trx_utils'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = melodyschool_sc_recent_news( array(
			'from_widget'		=> true,
		...
		if (!empty($output)) {
			...
			if ($widget_title) melodyschool_show_layout($before_title . $widget_title . $after_title);
	...
	function form($instance) {
		...
		$posts_types = melodyschool_get_list_posts_types(false);
		$categories = melodyschool_get_list_terms(false, melodyschool_get_taxonomy_categories_by_post_type($post_type));
		$news_styles = melodyschool_get_list_templates('news');
		?>
...
*/
if ( !function_exists( 'melodyschool_sc_recent_news' ) ) {
	function melodyschool_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (melodyschool_param_is_on($show_categories)) {
			if ( !melodyschool_storage_isset('categories_'.$category) ) {
				melodyschool_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = melodyschool_prepare_widgets_args(melodyschool_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (melodyschool_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (melodyschool_param_is_on($show_categories) && !melodyschool_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(melodyschool_param_is_on($show_categories) && !melodyschool_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = melodyschool_storage_get('categories_'.$category);
			if (melodyschool_param_is_on($show_categories) && !empty($cat_list)) {
		...
		$args = melodyschool_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	add_shortcode("trx_recent_news", "melodyschool_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('melodyschool_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_recent_news_reg_shortcodes_vc');
	function melodyschool_widget_recent_news_reg_shortcodes_vc() {
		
		$posts_types = melodyschool_get_list_posts_types(false);
		$categories = melodyschool_get_list_terms(false, melodyschool_get_taxonomy_categories_by_post_type('post'));
		$news_styles = melodyschool_get_list_templates('news');
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip(melodyschool_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $categories)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "count",
					...
					array(
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip(melodyschool_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip(melodyschool_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
