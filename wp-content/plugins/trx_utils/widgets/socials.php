...
// Theme init
if (!function_exists('melodyschool_widget_socials_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_widget_socials_theme_setup', 1 );
	function melodyschool_widget_socials_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_socials_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('melodyschool_widget_socials_load')) {
	add_action( 'widgets_init', 'melodyschool_widget_socials_load' );
	function melodyschool_widget_socials_load() {
		register_widget( 'melodyschool_widget_socials' );
	}
}

// Widget Class
class melodyschool_widget_socials extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => esc_html__('Show site logo and social links', 'trx_utils') );
		parent::__construct( 'melodyschool_widget_socials', esc_html__('MelodySchool - Show logo and social links', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? melodyschool_do_shortcode($instance['text']) : '';
            ...
            <?php
				if ($show_logo) {
					...
					if ($logo_image || $logo_text)
						melodyschool_show_logo($logo_image, false, false, false, $logo_text, $logo_slogan);
				}
				...
				if ($show_icons) {
					melodyschool_show_layout(melodyschool_sc_socials(array('size' => "tiny", 'shape' => 'round')));
				}
			?>
		</div>
	...
	function form( $instance ) {
		...
		<p>
            ...
            <?php
			melodyschool_show_layout(melodyschool_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
...
*/
if ( !function_exists( 'melodyschool_sc_widget_socials' ) ) {
	function melodyschool_sc_widget_socials($atts, $content=null){	
		...
		extract($atts);
		$type = 'melodyschool_widget_socials';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_socials' 
								. (melodyschool_exists_visual_composer() ? ' vc_widget_socials wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, melodyschool_prepare_widgets_args(melodyschool_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_socials', 'widget_socials') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_widget_socials', $atts, $content);
	}
	add_shortcode("trx_widget_socials", "melodyschool_sc_widget_socials");
}


// Add [trx_widget_socials] in the VC shortcodes list
if (!function_exists('melodyschool_widget_socials_reg_shortcodes_vc')) {
	function melodyschool_widget_socials_reg_shortcodes_vc() {
