...
// Theme init
if (!function_exists('melodyschool_widget_recent_posts_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_widget_recent_posts_theme_setup', 1 );
	function melodyschool_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('melodyschool_exists_visual_composer') && melodyschool_exists_visual_composer())
			add_action('melodyschool_action_shortcodes_list_vc','melodyschool_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('melodyschool_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'melodyschool_widget_recent_posts_load' );
	function melodyschool_widget_recent_posts_load() {
		register_widget('melodyschool_widget_recent_posts');
	}
}

// Widget Class
class melodyschool_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'trx_utils'));
		parent::__construct( 'melodyschool_widget_recent_posts', esc_html__('MelodySchool - Recent Posts', 'trx_utils'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), melodyschool_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'melodyschool_sc_widget_recent_posts' ) ) {
	function melodyschool_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'melodyschool_widget_recent_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_posts' 
								. (melodyschool_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, melodyschool_prepare_widgets_args(melodyschool_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('melodyschool_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	add_shortcode("trx_widget_recent_posts", "melodyschool_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('melodyschool_widget_recent_posts_reg_shortcodes_vc')) {
	function melodyschool_widget_recent_posts_reg_shortcodes_vc() {
