<?php
/**
 * MelodySchool Framework: social networks
 *
...
// Theme init
if (!function_exists('melodyschool_socials_theme_setup')) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_socials_theme_setup' );
	function melodyschool_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 'melodyschool_facebook_og_tags', 5 );

		} else {
            // Add extra fields in the user profile
            add_action( 'show_user_profile',		'melodyschool_add_fields_in_user_profile' );
            add_action( 'edit_user_profile',		'melodyschool_add_fields_in_user_profile' );

            // Save / update additional fields from profile
            add_action( 'personal_options_update',	'melodyschool_save_fields_in_user_profile' );
            add_action( 'edit_user_profile_update',	'melodyschool_save_fields_in_user_profile' );
        }
...
if (!function_exists('melodyschool_socials_theme_setup1')) {
    add_action('melodyschool_action_before_init_theme', 'melodyschool_socials_theme_setup1');
    function melodyschool_socials_theme_setup1()
    {

// List of social networks for site sharing and user profiles
        melodyschool_storage_set('share_links', array(
                ...
                'postila' => 'http://postila.ru/publish/?url={link}&agregator=melodyschool',
...
if (!function_exists('melodyschool_add_share_link')) {
	function melodyschool_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		melodyschool_storage_set('share_links', array_merge( melodyschool_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('melodyschool_show_share_links')) {
	function melodyschool_show_share_links($args) {
		if ( melodyschool_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => melodyschool_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => melodyschool_get_custom_option('show_share'),				// share block direction
			'caption' => melodyschool_get_custom_option('share_caption'),				// share block caption
			'share' => melodyschool_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
                ...
                if (empty($url)) $url = melodyschool_storage_get_array('share_links', $sn);
                ...
                $output .= '>'
					...
					. '</a>'
					. ($args['type']=='drop' ? '<i>' . trim(melodyschool_strtoproper($sn)) . '</i>' : '')
					. '</div>';
...
if (!function_exists('melodyschool_prepare_socials')) {
	function melodyschool_prepare_socials($list, $style='') {
		if (empty($style)) $style = melodyschool_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || melodyschool_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : melodyschool_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('melodyschool_get_twitter_data')) {
	function melodyschool_get_twitter_data($cfg) {
...
if (!function_exists('melodyschool_prepare_twitter_text')) {
	function melodyschool_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('melodyschool_get_twitter_followers')) {
	function melodyschool_get_twitter_followers($cfg) {
		$data = melodyschool_get_twitter_data($cfg); 
...
if (!function_exists('melodyschool_get_facebook_likes')) {
	function melodyschool_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(melodyschool_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (melodyschool_strpos($headers[0], '200')) {
			$json = melodyschool_fgc($url);
...
if (!function_exists('melodyschool_facebook_og_tags')) {
	//add_action( 'wp_head', 'melodyschool_facebook_og_tags', 5 );
	function melodyschool_facebook_og_tags() {
		global $post;
		if ( !is_singular() || melodyschool_storage_get('blog_streampage')) return;
...
if (!function_exists('melodyschool_get_feedburner_counter')) {
	function melodyschool_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(melodyschool_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (melodyschool_strpos($headers[0], '200')) {
			$xml = melodyschool_fgc($url);
...
if (!function_exists('melodyschool_add_fields_in_user_profile')) {
    function melodyschool_add_fields_in_user_profile( $user ) {
        ?>
        <h3><?php esc_html_e('User Position', 'melodyschool'); ?></h3>
        <table class="form-table">
            <tr>
                <th><label for="user_position"><?php esc_html_e('User position', 'melodyschool'); ?>:</label></th>
                <td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
                    <span class="description"><?php esc_html_e('Please, enter your position in the company', 'melodyschool'); ?></span>
                </td>
            </tr>
        </table>

        <h3><?php esc_html_e('Social links', 'melodyschool'); ?></h3>
        <table class="form-table">
            <?php
            ...
            if (is_array($social_list) && count($social_list) > 0) {
                foreach ($social_list as $soc) {
                    ...
                    if (!empty($sn)) {
                        ?>
                        <tr>
                            <th><label for="user_<?php echo esc_attr($sn); ?>"><?php melodyschool_show_layout(melodyschool_strtoproper($sn)); ?>:</label></th>
                            <td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
                                <span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'melodyschool'), melodyschool_strtoproper($sn)); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
...
if (!function_exists('melodyschool_save_fields_in_user_profile')) {
    function melodyschool_save_fields_in_user_profile( $user_id ) {
...
if (!function_exists('trx_utils_get_post_views')) {
    ...
    function trx_utils_get_post_views($default=0, $id=0){
        ...
        $count_key = melodyschool_storage_get('options_prefix').'_post_views_count';
...
if (!function_exists('trx_utils_get_post_likes')) {
    ...
    function trx_utils_get_post_likes($default=0, $id=0){
        global $wp_query;
        if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
        $count_key = melodyschool_storage_get('options_prefix').'_post_likes_count';
        $count = get_post_meta($id, $count_key, true);
        if ($count===''){
            delete_post_meta($id, $count_key);
            add_post_meta($id, $count_key, '0');
