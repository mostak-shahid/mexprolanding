...
function melodyschool_theme_customizer(option, val) {
    "use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = MELODYSCHOOL_STORAGE['co_add_params']!=undefined ? MELODYSCHOOL_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#melodyschool-customizer-styles-'+option).length > 0 ? jQuery('#melodyschool-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="melodyschool-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
