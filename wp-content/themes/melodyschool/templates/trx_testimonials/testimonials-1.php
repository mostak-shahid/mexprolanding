...
if ( !function_exists( 'melodyschool_template_testimonials_1_theme_setup' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_template_testimonials_1_theme_setup', 1 );
	function melodyschool_template_testimonials_1_theme_setup() {
		melodyschool_add_template(array(
			'layout' => 'testimonials-1',
			'template' => 'testimonials-1',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 1/', 'melodyschool'),
			'thumb_title'  => esc_html__('Avatar (middle)', 'melodyschool'),
			'w'		 => 184,
			'h'		 => 184
		));
	}
...
// Template output
if ( !function_exists( 'melodyschool_template_testimonials_1_output' ) ) {
	function melodyschool_template_testimonials_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
                <?php if ($post_options['photo']) { ?>
                    <div class="sc_testimonial_avatar"><?php melodyschool_show_layout($post_options['photo']); ?></div>
                <?php } ?>
				<div class="sc_testimonial_content"><?php melodyschool_show_layout($post_data['post_content']); ?></div>
