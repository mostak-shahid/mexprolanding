...
if ( !function_exists( 'melodyschool_template_list_theme_setup' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_template_list_theme_setup', 1 );
	function melodyschool_template_list_theme_setup() {
		melodyschool_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'melodyschool')
			));
	}
}

// Template output
if ( !function_exists( 'melodyschool_template_list_output' ) ) {
	function melodyschool_template_list_output($post_options, $post_data) {
		...
		$title = '<li class="sc_blogger_item sc_list_item'.($columns > 1 ? ' column-1_'.esc_attr($columns) : '').'">'
			...
			. '</li>';
		melodyschool_show_layout($title);
	}
}
?>