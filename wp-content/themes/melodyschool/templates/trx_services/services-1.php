...
if ( !function_exists( 'melodyschool_template_services_1_theme_setup' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_template_services_1_theme_setup', 1 );
	function melodyschool_template_services_1_theme_setup() {
		melodyschool_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'melodyschool'),
...
// Template output
if ( !function_exists( 'melodyschool_template_services_1_output' ) ) {
	function melodyschool_template_services_1_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_services_item_content">
					...
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
							?><h3 class="sc_services_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php melodyschool_show_layout($post_data['post_title']); ?></a></h3><?php
						} else {
							?><h3 class="sc_services_item_title"><?php melodyschool_show_layout($post_data['post_title']); ?></h3><?php
						}
					}
					?>
