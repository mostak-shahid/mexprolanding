...
if ( !function_exists( 'melodyschool_template_news_magazine_theme_setup' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_template_news_magazine_theme_setup', 1 );
	function melodyschool_template_news_magazine_theme_setup() {
		melodyschool_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'melodyschool'),
...
// Template output
if ( !function_exists( 'melodyschool_template_news_magazine_output' ) ) {
	function melodyschool_template_news_magazine_output($post_options, $post_data) {
				...
				<div class="post_header entry-header">
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php melodyschool_show_layout($post_data['post_title']); ?></a></<?php echo esc_attr($title_tag); ?>>
							<?php
						} else {
							?>
							<<?php echo esc_attr($title_tag); ?> class="post_title entry-title"><?php melodyschool_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
							<?php
						}
					}
					
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						?><div class="post_meta"><span class="post_meta_author"><?php melodyschool_show_layout($post_data['post_author_link']); ?></span><?php
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(melodyschool_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
