...
if ( !function_exists( 'melodyschool_template_news_announce_theme_setup' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_template_news_announce_theme_setup', 1 );
	function melodyschool_template_news_announce_theme_setup() {
		melodyschool_add_template(array(
			'layout' => 'news-announce',
			'template' => 'news-announce',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Announce/', 'melodyschool')
		));

		// Add thumb sizes into list
		melodyschool_add_thumb_sizes( array( 'thumb_slug' => 'full', 'add_image_size' => false, 'w' => 1170, 'h' => 659 ) );
		melodyschool_add_thumb_sizes( array( 'thumb_slug' => 'big', 'add_image_size' => false, 'w' => 770, 'h' => 434 ) );
		melodyschool_add_thumb_sizes( array( 'thumb_slug' => 'medium', 'add_image_size' => false, 'w' => 770, 'h' => 217 ) );
		melodyschool_add_thumb_sizes( array( 'thumb_slug' => 'small', 'add_image_size' => false, 'w' => 370, 'h' => 209 ) );
	}
}

// Template output
if ( !function_exists( 'melodyschool_template_news_announce_output' ) ) {
	function melodyschool_template_news_announce_output($post_options, $post_data) {
		...
		$post_data['post_thumb'] = melodyschool_get_resized_image_tag($post_data['post_attachment'], $thumb_sizes['w'], $post_data['post_type']=='product' && melodyschool_get_theme_option('crop_product_thumb')=='no' ? null :  $thumb_sizes['h']);
		?><article id="post-<?php echo esc_attr($post_data['post_id']); ?>"
			...
			<div class="post_featured">
				...
				<div class="post_info">
					...
					<h5 class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>" rel="bookmark"><?php melodyschool_show_layout($post_data['post_title']); ?></a></h5>
					<?php if ( in_array($post_data['post_type'], array('post', 'attachment')) ) { ?>
						<div class="post_meta">
							<span class="post_meta_author"><?php melodyschool_show_layout($post_data['post_author_link']); ?></span>
