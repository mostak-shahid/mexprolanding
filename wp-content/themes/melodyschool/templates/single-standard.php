...
if ( !function_exists( 'melodyschool_template_single_standard_theme_setup' ) ) {
	add_action( 'melodyschool_action_before_init_theme', 'melodyschool_template_single_standard_theme_setup', 1 );
	function melodyschool_template_single_standard_theme_setup() {
		melodyschool_add_template(array(
			'layout' => 'single-standard',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'melodyschool'),
			'thumb_title'  => esc_html__('Fullwidth image (crop)', 'melodyschool'),
			'w'		 => 1170,
			'h'		 => 659
		));
	}
...
// Template output
if ( !function_exists( 'melodyschool_template_single_standard_output' ) ) {
	function melodyschool_template_single_standard_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && melodyschool_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_attr($title_tag); ?> itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php melodyschool_show_layout($post_data['post_title']); ?></<?php echo esc_attr($title_tag); ?>>
		<?php 
		}
		...
		if (!$sidebar_present) melodyschool_close_wrapper();
