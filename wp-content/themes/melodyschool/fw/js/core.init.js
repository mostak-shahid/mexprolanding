...
function melodyschool_ready_actions() {
	...
	jQuery('.widget_subcategories').on('change', 'select', function() {
		...
		if ( dropdown.options[dropdown.selectedIndex].value > 0 ) {
			location.href = MELODYSCHOOL_STORAGE['site_url'] + "/?cat="+dropdown.options[dropdown.selectedIndex].value;
		}
	});
	...
	if (jQuery('.header_mobile').length > 0) {
		...
		jQuery('.header_mobile .menu_button').on('click', function(){
			...
			// Fix for Safari
			if (melodyschool_browser_is_ios() && jQuery('body').hasClass('menu_mobile')) {
	...
	if (MELODYSCHOOL_STORAGE['menu_hover']=='slide_line' || MELODYSCHOOL_STORAGE['menu_hover']=='slide_box') {
		setTimeout(function() {
			"use strict";
			jQuery('#menu_main').spasticNav({
				style: MELODYSCHOOL_STORAGE['menu_hover']=='slide_line' ? 'line' : 'box',
				color: MELODYSCHOOL_STORAGE['accent1_hover'],
				colorOverride: false
			});
		}, 500);
	}
...
// Inc/Dec quantity on buttons inc/dec
	...
	jQuery('.bookmarks_add').on('click', function(e) {
		...
		var list = melodyschool_get_cookie('melodyschool_bookmarks');
		var exists = false;
		if (list) {
			try {
		...
		if (!exists) {
			var message_popup = melodyschool_message_dialog('<label for="bookmark_title">'+MELODYSCHOOL_STORAGE['strings']['bookmark_title']+'</label><br><input type="text" id="bookmark_title" name="bookmark_title" value="'+title+'">', MELODYSCHOOL_STORAGE['strings']['bookmark_add'], null,
				function(btn, popup) {
                    ...
                    melodyschool_set_cookie('melodyschool_bookmarks', JSON.stringify(list), 365);
	...
	// AJAX views counter
	if (MELODYSCHOOL_STORAGE['ajax_views_counter'] !== undefined) {
		setTimeout(function(){
			jQuery.post(MELODYSCHOOL_STORAGE['ajax_url'], {
				action: 'post_counter',
				nonce: MELODYSCHOOL_STORAGE['ajax_nonce'],
				post_id: MELODYSCHOOL_STORAGE['ajax_views_counter']['post_id'],
				views: MELODYSCHOOL_STORAGE['ajax_views_counter']['post_views']
			});
			}, 10);
	}
...
function melodyschool_resize_actions() {
	...
	// Reset stored value
	if (!jQuery('body').hasClass('top_panel_fixed')) MELODYSCHOOL_STORAGE['top_panel_height'] = 0;
...
function melodyschool_init_sfmenu(selector) {
	...
	jQuery(selector).show().each(function() {
		...
		jQuery(this).addClass('inited').superfish({
			...
			speed: 		MELODYSCHOOL_STORAGE['css_animation'] ? 500 : 200,
			speedOut:	MELODYSCHOOL_STORAGE['css_animation'] ? 500 : 200,
			autoArrows: false,
			dropShadows: false,
...
function melodyschool_show_popup(popup) {
    "use strict";
	if (popup.css('display')=='none') {
		if (false && MELODYSCHOOL_STORAGE['css_animation'])
