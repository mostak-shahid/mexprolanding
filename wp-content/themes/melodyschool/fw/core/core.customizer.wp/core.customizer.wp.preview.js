...
( function( $ ) {
	...
	if (!melodyschool_previewer_vars['need_refresh']) {
		var $style = $('#melodyschool-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<style type="text/css" id="melodyschool-customizer-css"></style>' ).find('#melodyschool-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			melodyschool_set_cookie(melodyschool_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
